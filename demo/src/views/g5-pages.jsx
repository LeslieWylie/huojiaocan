...
export function ResearchLedgerPage() {
  ...
  const load = async () => {
    ...
    setBusy(true); setError(''); setLedger({ items: [], summary: {} });
    try {
      const data = await rootRequest('/api/assets/research');
      if (requestId === loadRequest.current) setLedger(data.ledger || { items: [], summary: {} });
    } catch (err) {
      if (requestId !== loadRequest.current) return;
      if (['auth_required', 'auth_invalid'].includes(requestCode(err))) { location.href = `/login/?next=${encodeURIComponent('/research/')}`; return; }
      setError(askErrorMessage(err));
    } finally { if (requestId === loadRequest.current) setBusy(false); }
  };
  useEffect(() => {
    setLedger({ items: [], summary: {} }); setBusy(true); setError('');
    if (!userId) { if (session === null) location.href = `/login/?next=${encodeURIComponent('/research/')}`; }
    else load();
    return () => { loadRequest.current += 1; };
  }, [userId]);
  const summary = ledger.summary || {};
  return <div className="view-stack research-ledger-page"><section className="hero compact-hero research-hero"><div><Badge tone="gold"><FileText/> 教研问题簿</Badge><h1>不统计做了多少份方案，<br/><em>只记录一个问题怎样被课堂推进</em></h1><p>每次“一课一研”是一份课堂样本，两次同篇目实践可以形成对照，教师确认后才成为教研命题。问题簿只显示下一步，不用生成量代替教研进展。</p><div className="hero-actions"><a href="/assets/"><Archive/>打开教研资产</a><a href="/study/"><Microscope/>整理一课一研</a></div></div><div className="research-seal"><strong>{busy || error ? '—' : summary.lessonCount || 0}</strong><span>条教研问题线</span><em>{busy ? '正在读取课堂记录' : error ? '课堂记录暂未读到' : summary.needsValidationCount ? `${summary.needsValidationCount} 条等待继续验证` : '等待课堂继续推进'}</em></div></section>{error && <section className="cards-alert" role="alert"><div className="cards-alert-icon"><CircleAlert/></div><div className="cards-alert-copy"><b>教研问题簿暂时没有读取完成</b><p>{error}</p></div><div className="cards-alert-actions"><button type="button" onClick={load}><RefreshCw/>重新读取</button></div></section>}{busy ? <section className="panel research-empty"><Activity/><h2>正在整理你的教研问题线</h2><p>只读取当前账号中由教师确认的课堂记录和教研命题。</p></section> : error ? null : ledger.items?.length ? <><section className="research-progress-strip" aria-label="教研问题推进情况"><div><span>课堂样本</span><strong>{summary.sampleCount || 0}</strong><small>教师确认的一课一研</small></div><ArrowRight/><div><span>可开始对照</span><strong>{summary.readyToCompareCount || 0}</strong><small>已有两次同篇目课堂</small></div><ArrowRight/><div><span>已确认命题</span><strong>{summary.confirmedHypothesisCount || 0}</strong><small>写清适用边界与验证方式</small></div></section><section className="research-ledger-list">{ledger.items.map(item => { const stage = researchStage(item.nextAction), latestHypothesis = item.comparisons.find(comparison => comparison.status === 'confirmed' && !comparison.stale); return <article className="panel research-line" key={item.lessonIdentity}><header><div><span>教研问题线</span><h2>{item.lessonTitle}</h2><p>{item.samples.length} 次确认课堂 · {item.comparisons.length} 次同课对照</p></div><Badge tone={stage.tone}>{stage.label}</Badge></header><div className="research-line-body"><section><b>课堂样本</b><div className="research-sample-list">{item.samples.slice(0, 3).map((sample, index) => <a href={`/study/?draftId=${encodeURIComponent(sample.draftId)}`} key={sample.draftId}><span>{String(index + 1).padStart(2, '0')}</span><div><strong>{sample.label}</strong><p>{sample.finding || '教师已确认本次课堂记录。'}</p><small>{sample.confirmedAt ? new Date(sample.confirmedAt).toLocaleDateString() : '确认时间未知'}</small></div><ArrowRight/></a>)}</div></section><section className={latestHypothesis ? 'research-hypothesis confirmed' : 'research-hypothesis'}><b>{latestHypothesis ? '当前教研命题' : '目前还不能形成跨课堂结论'}</b>{latestHypothesis ? <><blockquote>{latestHypothesis.transferableFinding || '教师已经确认本次同课对照。'}</blockquote><p><span>下一次验证</span>{latestHypothesis.nextExperiment || '等待教师补充下一次验证方式。'}</p><a href={`/observation/?left=${encodeURIComponent(latestHypothesis.leftId)}&right=${encodeURIComponent(latestHypothesis.rightId)}`}><ClipboardCheck/>生成听评课观察单 <ArrowRight/></a></> : <p>{item.samples.length > 1 ? '已经具备两次课堂样本，可以开始并列事实。' : '需要再完成一次同篇目课堂，并保持观察指标一致。'}</p>}</section></div><footer><div><b>下一步</b><p>{item.nextAction.note}</p></div><a className="primary" href={item.nextAction.href}>{item.nextAction.label} <ArrowRight/></a></footer></article>; })}</section></> : <section className="panel research-empty"><div className="empty-orbit"><FileText/></div><h2>问题簿还没有课堂样本</h2><p>先完成一次课堂记录和课后复盘，再把“一课一研”确认下来。这里会自动形成第一条教研问题线。</p><div><a className="primary" href="/assets/">从教研资产开始</a><a href="/library/">选择教材篇目</a></div></section>}</div>;
}
...
export function AssetsPage() {
  ...
  const hasFilters = Boolean(query.trim() || favoriteOnly || tagFilter);
  const clearFilters = () => { setQuery(''); setFavoriteOnly(false); setTagFilter(''); };
  const loadRequest = useRef(0);
  const load = async () => { const requestId = ++loadRequest.current; setBusy(true); setError(''); setLoadError(false); try { const params = new URLSearchParams(); if (query.trim()) params.set('q', query.trim()); if (favoriteOnly) params.set('favorite', 'true'); if (tagFilter) params.set('tag', tagFilter); const data = await rootRequest(`/api/assets${params.toString() ? `?${params}` : ''}`); if (requestId !== loadRequest.current) return; setAssets(Array.isArray(data.assets) ? data.assets : []); setTagOptions(Array.isArray(data.tags) ? data.tags : []); } catch (err) { if (requestId !== loadRequest.current) return; if (['auth_required', 'auth_invalid'].includes(requestCode(err))) { location.href = `/login/?next=${encodeURIComponent('/assets/')}`; return; } setLoadError(true); setError(askErrorMessage(err)); } finally { if (requestId === loadRequest.current) setBusy(false); } };
  useEffect(() => { loadRequest.current += 1; setBusy(true); const timer = setTimeout(load, query.trim() ? 240 : 0); return () => { clearTimeout(timer); loadRequest.current += 1; }; }, [query, favoriteOnly, tagFilter]);
