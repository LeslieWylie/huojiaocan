import PrivateMaterialPreview, { isPrivateMaterial } from '../private-material-preview.jsx';
...
export function IngestPage() {
  ...
  const createTask=async()=>{
    ...
    try{
      ...
      location.assign(navigationHref('jobs', `/jobs/?jobId=${encodeURIComponent(jobId)}&doc=${encodeURIComponent(documentId)}`));
  ...
  return <div className="view-stack"><section className="hero compact-hero"><div><Badge tone="green"><Upload/> 教材导入向导</Badge><h1>原始文件不可变保存，<br/>页面内容按教材页码逐页处理</h1><p>扫描 PDF、页面文字 PDF 和混合 PDF 都可导入；公开界面始终展示原始教材页面。</p></div><div className="wizard-steps">{['选择文件','文档预检','确认策略','创建任务'].map((x,i)=><span className={step>i?'active':''} key={x}><b>{i+1}</b>{x}</span>)}</div></section>
    ...
    {step===1?<section className="panel upload-panel"><label className="dropzone" onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); setFile(event.dataTransfer.files?.[0] || null); setError(''); }}><Upload/><h2>拖入 PDF，或点击选择文件</h2><p>支持页面文字、扫描页与混合文档。原始教材不会被解析结果覆盖。</p><input type="file" accept="application/pdf" onChange={e=>setFile(e.target.files?.[0]||null)}/><span>{file?file.name:'选择 PDF 文件'}</span></label><button type="button" className="primary wide-action" onClick={preflight} disabled={!file}>检查当前文件</button></section>:<><section className="panel"><SectionHead icon={FileCheck2} eyebrow="文档预检" title={displayFile.name} note="当前页面先检查文件；提交后会逐页读取内容并建立目录。"/><div className="preflight-grid"><Stat icon={FileText} label="文件大小" value={`${(displayFile.size/1024/1024).toFixed(1)} MB`} note="登记后校验文件完整性"/><Stat icon={BookOpen} label="预计页数" value="提交后读取" note="教材页码由解析任务确认"/><Stat icon={FileSearch} label="页面文字" value="逐页检查" note="质量合格时直接使用" tone="green"/><Stat icon={Eye} label="扫描页面" value="自动判断" note="只识别缺失或质量不足的页面" tone="gold"/></div><div className="preview-strip">{['起始页','中段页','末页'].map(label=><div key={label}><div className="preview-paper"><FileText/><span>原始教材<br/>{label}</span></div><b>任务完成后可核验</b></div>)}</div></section><section className="panel confirm-grid"><div><label htmlFor="import-document-kind">文档分类</label><select id="import-document-kind" value={kind} onChange={e=>setKind(e.target.value)}><option value="textbook">学生教材</option><option value="teacher_guide">教师教学用书</option><option value="other">其他教学资料</option></select></div><div><label htmlFor="import-extraction-policy">提取策略</label><select id="import-extraction-policy" value={policy} onChange={e=>setPolicy(e.target.value)}><option value="auto">自动判断（推荐）</option><option value="native">仅使用页面文字</option><option value="ocr">强制重新识别页面</option></select></div><div className="policy-note"><CheckCircle2/><span><b>教材页码保护</b><small>任何解析和人工修正都不会改变 第几页。</small></span></div><button type="button" disabled={working} onClick={() => { setStep(1); setError(''); }}>重新选择文件</button><button type="button" className="primary" onClick={createTask} disabled={working}>{working?'正在登记并创建任务…':'确认并开始处理'} <ArrowRight/></button>{created&&<small>已开始处理，正在进入进度页面。</small>}</section></>}
  </div>;
}
...
export function InspectPage() {
  ...
  const loadPage=async(target=page,signal)=>{
    ...
    setLoading(true);setError('');setLoadedPageKey('');setRecord(null);setRetrievalText('');setTitle('');setPrintedPage('');setSectionPath('');
    try {
      const data=await request(`/page/${encodeURIComponent(doc)}/${target}`,{signal});
      if(!current())return;
      if(!inspectedPageMatches(data,doc,target))throw new Error('page_identity_mismatch');
      applyPage(data);setLoadedPageKey(key);
    } catch(err) { if(current())setError(`暂时无法读取第 ${target} 页，请稍后重试。`); }
  ...
  const ocrNote=!record ? '本页尚未成功读取，不会显示或保存其他页内容。' : record?.ocrError
  ...
  return <div className="view-stack"><section className="panel inspect-toolbar"><div><Badge tone={info.tone}>{info.short}</Badge><h1>页面校正</h1><p>原始教材不可修改；这里只调整用于搜索的页面文字、标题、章节和书页码。</p></div><div><button type="button" onClick={()=>setPage(p=>Math.max(1,p-1))}>上一页</button><input aria-label="教材页码" inputMode="numeric" value={page} onChange={e=>setPage(Math.max(1,Number(e.target.value)||1))}/><button type="button" onClick={()=>setPage(p=>p+1)}>下一页</button><a href={navigationHref('document', `/document/?doc=${encodeURIComponent(doc)}&page=${page}&return=${encodeURIComponent(navigationHref('inspect', `/inspect/?doc=${encodeURIComponent(doc)}&page=${page}`))}`)}>核验原页 <ExternalLink/></a></div></section>{error&&<div className="ask-error"><CircleAlert/>{error}<button type="button" disabled={loading} onClick={()=>loadPage(page)}>重试加载</button></div>}{notice&&<div className="quality-box"><CheckCircle2/><span>{notice}{rerunJobId && <a href={navigationHref('jobs', `/jobs/?doc=${encodeURIComponent(doc)}&jobId=${encodeURIComponent(rerunJobId)}&page=${page}`)}>查看本页处理进度 <ArrowRight/></a>}</span></div>}<div className="inspect-layout"><section className="panel original-preview"><header><b>原始教材· 教材页码 {page}</b><Badge tone="green">唯一可核验的依据</Badge></header>{isPrivateMaterial(doc)?<PrivateMaterialPreview documentId={doc} pageNumber={page} title={`原始材料 第 ${page} 页`}/>:rawPdfUrl?<iframe key={`${doc}-${page}`} title="原始教材页面" src={pdfPageUrl(rawPdfUrl,page)}/>:<div className="index-empty"><FileText/><b>正在读取原始教材</b><p>页面信息加载后会在这里显示对应原页。</p></div>}</section><section className="panel extraction-editor"><SectionHead icon={FileSearch} eyebrow="用于教材搜索的文字" title={loading?'正在读取页面…':'当前生效文字'}/><div className="source-tabs"><button type="button" className={source==='native'?'active':''} onClick={()=>setSource('native')}>页面文字</button><button type="button" className={source==='ocr'?'active':''} onClick={()=>setSource('ocr')}>扫描页文字</button><button type="button" className={source==='retrieval'?'active':''} onClick={()=>setSource('retrieval')}>当前生效文字</button></div><p className="inspect-source-note">{ocrNote}</p><textarea value={shownText} readOnly={source!=='retrieval'||!canEdit} onChange={e=>setRetrievalText(e.target.value)} placeholder={loading?'正在加载…':!canEdit?'本页尚未成功读取，不能保存调整':source==='ocr'&&!ocrText?'本页没有页面识别文字':'该文本来源暂无内容'}/><div className="editor-fields"><label>页面标题<input value={title} disabled={!canEdit} onChange={e=>setTitle(e.target.value)}/></label><label>书页码<input value={printedPage} disabled={!canEdit} onChange={e=>setPrintedPage(e.target.value)}/></label><label>章节路径<input value={sectionPath} disabled={!canEdit} onChange={e=>setSectionPath(e.target.value)}/></label></div><div className="quality-box"><CheckCircle2/><span><b>质量状态：{record?.textQualityStatus||record?.qualityStatus||'待读取'}</b><small>页面文字来源：{sourceName} · 教材页码 {page} 保持不变</small></span></div><div className="editor-actions"><label><input type="checkbox" checked={included} disabled={!canEdit} onChange={e=>setIncluded(e.target.checked)}/>纳入教材搜索</label><button type="button" onClick={rerun} disabled={rerunning||loading}><RefreshCw/>{rerunning?'正在创建任务':'重新读取当前页'}</button><button type="button" className="primary" onClick={save} disabled={saving||!canEdit}>{saving?'正在保存':'保存页面调整'}</button></div></section></div></div>;
}
...
export function CurriculumAlignmentPage() {
  ...
  const alignmentParams = new URLSearchParams(params);
  if (!draftId && lessonTitle.trim() !== (params.get('lesson') || '').trim()) {
    // A manually named lesson has no verified ownership of the old position.
    for (const key of ['page', 'node', 'unit']) alignmentParams.delete(key);
  }
  alignmentParams.set('lesson', lessonTitle);
  const alignmentReaderReturn = navigationHref('alignment', '/alignment/', alignmentParams.toString());
  useEffect(() => { if (!draftId) replaceWorkspaceUrl(alignmentReaderReturn); }, [draftId, alignmentReaderReturn]);
  ...
  return <div className="view-stack alignment-page">
    ...
    })}</section> : <section className="panel alignment-missing"><Target/><b>先确定要核对的篇目</b><p>也可以从教材库选定篇目后进入本页，系统会自动带入篇名和当前教材范围。</p><a className="primary" href={navigationHref('library', '/library/')}>从教材库选择篇目</a></section>}
    <section className="panel alignment-summary"><div><span>本页边界</span><h2>{report.warning}</h2><p>{draftId ? '确认后把课标依据写入当前方案；教师用书处理和学生教材原文仍分别核验。' : '当前尚无方案，本页选择未保存；开始备课后需在方案中重新核对并确认课标依据。'}</p>{notice && <strong>{notice}</strong>}</div><div>{draftId && session ? <button type="button" className="primary" disabled={saving || busy} onClick={save}>{saving ? '正在保存…' : '确认本课课标依据'}</button> : <a className="primary" href={session ? navigationHref('ask', `/ask/?lesson=${encodeURIComponent(lessonTitle)}`) : `/login/?next=${encodeURIComponent(`/alignment/?lesson=${encodeURIComponent(lessonTitle)}`)}`}>{session ? '按当前篇目开始备课' : '登录后保存对齐'}</a>}{draftId && <a href={`/cards/?draftId=${encodeURIComponent(draftId)}`}>回到本课方案 <ArrowRight/></a>}</div></section>
  </div>;
}
