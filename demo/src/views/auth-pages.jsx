...
export function LoginPage({ callback: initialCallback = null }) {
  ...
  useEffect(() => {
    ...
    globalThis.history?.replaceState?.(null, document.title, `${location.pathname}${clean.toString() ? `?${clean}` : ''}`);
  }, [params]);
...
export function SettingsPage() {
  ...
  return <div className="view-stack settings-page"><section className="hero compact-hero"><div><Badge tone="blue"><ShieldCheck/> AI 设置</Badge><h1>配置你的 AI 连接<br/><em>再开始生成备课方案</em></h1><p>备课问答与三卡仅使用你自己的 DeepSeek 密钥。页面只显示末四位和测试状态，完整密钥不会返回浏览器。</p></div></section><section className="panel"><SectionHead icon={Plus} eyebrow="添加我的智能连接" title="配置 DeepSeek 官方接口" note="仅支持已接入的智能模型。这里保存的是你的智能连接，不要粘贴系统配置密钥。"/><form onSubmit={add} className="key-form"><label>模型<select value={model} onChange={event => setModel(event.target.value)}><option value="deepseek-v4-flash">标准智能模型</option><option value="deepseek-v4-pro">增强智能模型</option></select></label><label>DeepSeek 密钥<input type="password" value={apiKey} onChange={event => setApiKey(event.target.value)} placeholder="sk-…" autoComplete="off"/></label><button type="submit" className="primary" disabled={busy || !apiKey.trim()}>添加并保存</button></form>{error && <div className="ask-error"><CircleAlert/>{error}</div>}{message && <div className="quality-box"><CheckCircle2/>{message}</div>}</section><section className="panel"><SectionHead icon={ShieldCheck} eyebrow="已保存的我的智能连接" title="切换与测试" note="完整密钥不会返回浏览器。请先测试连接，再在备课问答中选择它。"/><div className="key-list">{keys.length ? keys.map(item => <article className="key-row" key={item.id}><div><b>{item.keyHint || 'DeepSeek 密钥'}</b><small>{item.model} · {item.lastTestStatus === 'valid' ? '测试通过' : item.lastTestStatus === 'invalid' ? '测试失败' : '尚未测试'}</small></div><Badge tone={item.isActive ? 'green' : 'neutral'}>{item.isActive ? '当前使用' : '未启用'}</Badge><button type="button" disabled={busy} onClick={() => action(item.id, 'test')}>测试</button>{!item.isActive && <button type="button" disabled={busy} onClick={() => action(item.id, 'activate')}>设为当前</button>}<button type="button" disabled={busy} onClick={() => remove(item.id)}>删除</button></article>) : <p className="muted">还没有保存我的智能连接。请先添加并测试 DeepSeek 密钥，再开始备课问答。</p>}</div></section><div className="hero-actions settings-return"><a className="primary" href="/ask/"><MessageCircle/>返回备课问答</a><a href="/"><Route/>返回教学任务</a></div></div>;
}
