...
export function firstLessonNode(nodes = []) {
  for (const node of nodes) {
    if (/^\s*\d+\s+/u.test(String(node?.title || '')) && Number(node?.startPage || node?.pageRange?.start) > 0) return node;
    const child = firstLessonNode(Array.isArray(node?.children) ? node.children : []);
    if (child) return child;
  }
  return null;
}
...
export function LibraryPage() {
  ...
  const firstLesson = firstLessonNode(tree);
    ...
    <div className="view-stack index-page">
      ...
      <div className={`index-workspace ${searched || busy ? 'search-active' : 'catalog-reading'}`}>
        ...
        <section className="index-reader">
          ...
          <article className="library-pdf-article"><div className="library-pdf-meta"><span>原始教材是唯一可核验的依据</span><b>第 {pageNo} 页 · 书页 {page?.printedPage || "未标注"}</b></div>{isPrivateMaterial(doc) ? <PrivateMaterialPreview documentId={doc} pageNumber={pageNo} title={`原始材料 第 ${pageNo} 页`}/> : !hasReadingTarget && pageNo === 1 ? <div className="library-reader-start"><BookOpen/><span>从教材目录开始</span><h3>先选具体篇目，再阅读原页</h3><p>左侧目录负责定位篇目；选中后，这里会同步打开准确的教材物理页，并把篇目和页码带入备课问答。</p><ol><li><b>01</b>选择篇目</li><li><b>02</b>核对原页</li><li><b>03</b>开始备课</li></ol>{firstLesson && <button type="button" onClick={() => pick(firstLesson)}>打开第一篇《{String(firstLesson.title).replace(/^\s*\d+\s*/u, '')}》</button>}</div> : pageBusy && !pagePdf ? <div className="index-empty" role="status"><Activity/><b>正在打开教材第 {pageNo} 页</b><p>目录与页码保持不变，页面读取完成后会自动显示。</p></div> : pagePdf && !pdfError ? <Suspense fallback={<p role="status">正在准备教材原页…</p>}><PdfPagePreview key={`${doc}-${pageRetry}`} title={`${currentDoc?.short || "教材"} 第 ${pageNo} 页`} url={pagePdf} pageNumber={pageNo} onError={() => setPdfError(true)}/></Suspense> : <div className="index-empty"><FileText/><b>{pageError || '当前教材页面暂时无法显示'}</b><p>可以重试当前页，或在新窗口打开原始 PDF。</p><div className="cards-alert-actions"><button type="button" onClick={() => { setPdfError(false); setPageRetry(value => value + 1); }}>重试当前页</button>{pagePdf && <a className="primary" href={pdfPageUrl(pagePdf,pageNo)} target="_blank" rel="noreferrer">新窗口打开</a>}</div></div>}</article>
        </section>
