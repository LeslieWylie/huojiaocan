...
import { readAiConnectionSelection } from '../ai-connection-selection.js';
...
import { normalizeBoardCards as withBoardPlan, resolveBoardQuestion } from '../../shared/board-presentation.js';
...
function CardLockConfirmation({ card, dirty, pending, error, onCancel, onConfirm, returnFocusRef }) {
  ...
  const keepFocusInside = event => {
    ...
    const buttons = [...dialogRef.current.querySelectorAll('button')].filter(button => !button.disabled);
  ...
  return <dialog ref={dialogRef} tabIndex={-1} className="panel card-lock-dialog" aria-labelledby="card-lock-title" aria-describedby="card-lock-description" aria-busy={pending} onKeyDown={keepFocusInside} onCancel={event => { event.preventDefault(); if (!pending) onCancel(); }}>
    <h2 id="card-lock-title">锁定“{card?.title}”？</h2>
    <p id="card-lock-description">锁定后不能编辑或重新生成本卡；需要调整时，可以复制为新版本。{dirty ? '确认后会先保存当前三卡修改，再锁定本卡。' : '确认将当前内容作为课堂版本。'}</p>
    {error && <p role="alert">锁定未完成，编辑内容仍保留。{error}</p>}
    <footer><button type="button" autoFocus disabled={pending} onClick={onCancel}>取消</button><button type="button" className="primary" disabled={pending} onClick={onConfirm}>{pending ? '正在锁定…' : '确认锁定'}</button></footer>
  </dialog>;
}

function cardsReadyForClassroom(draft, cards, dirty = false) {
  const state = deriveTeacherDraftState({ draft, cards, dirty });
  return state.teacherConfirmed && !state.unsavedChanges
    && ['board', 'question', 'assessment'].every(type => cards.some(card => card.type === type && card.status === 'locked' && card.items?.length));
}
...
export function Cards() {
  ...
  const [lockCandidate, setLockCandidate] = useState(null);
  ...
  const cardsBlocked = editorBusy || Boolean(recoveryConflict || lockCandidate);
  ...
  useEffect(() => {
    ...
    setSaving(false); setLocking(''); setLockCandidate(null); setGenerating('');
    ...
    rootRequest('/api/drafts/' + draftId)
      .then(data => {
        ...
        const launchClassroom = params.get('classroom') === '1' && nextRun.status !== 'pending_review'
          && !edits && cardsReadyForClassroom(next, loadedCards);
        if (params.get('classroom') === '1' && !launchClassroom) updateClassroomUrl(false);
  ...
  useEffect(() => {
    if (!generating) { setGenerationElapsed(0); return undefined; }
  ...
  const confirmAndGenerate = async () => {
    ...
    const keyId = readAiConnectionSelection(getSession()?.user?.id) || '';
    try {
  ...
  const lock = async card => {
    if (!card || editorBusy || recoveryConflict || cardRequestRef.current || generationRequestRef.current || card.status === 'locked' || lockCandidate !== card.id) return;
    const loadId = cardsLoadRef.current;
  ...
  const startClassroom = async () => {
    if (!classroomReady || cardsBlocked) return;
  ...
  const closeClassroom = async () => {
    if (classroomSaveRef.current) return;
    ...
    if (document.fullscreenElement) {
      ...
      catch { setClassroomNotice('未能退出全屏，请先退出全屏后再关闭课堂模式。'); return; }
    }
  ...
  const changeClassroomStage = nextStage => {
    ...
    if (classroomRun.status === 'confirmed') { setRevealed(stage); return; }
...
${sourceNote}`;
  ...
  // draft.question is the teacher's authoring request, not necessarily a
  // classroom question. Do not promote it when neither plan supplies one.
  const boardCoreQuestion = resolveBoardQuestion(board?.boardPlan?.coreQuestion, draft?.answer?.lesson?.coreQuestion);
  const boardWritingPlan = useMemo(() => buildBoardWritingPlan({
    title: lessonTitle,
    coreQuestion: boardCoreQuestion,
    items: board?.items || [],
    blankZones: board?.boardPlan?.blankZones || [],
    boardPlan: board?.boardPlan
  }), [lessonTitle, boardCoreQuestion, board?.items, board?.boardPlan]);
  const CARD_META = {
  ...
  const classroomReady = cardsReadyForClassroom(draft, cards, planDirty || dirty);
  ...
  const classroomButtonCopy = classroomReady ? classroomCta : planDirty || !workflowState.teacherConfirmed || !workflowState.cardsGenerated ? '继续：核对并生成三卡' : dirty ? '继续：保存三卡修改' : !allCardsLocked ? '继续：检查并锁定三卡' : '继续：检查并补全三卡';
  const continueClassroomFlow = () => {
    if (!draft || cardsBlocked) return;
    if (classroomReady) {
      startClassroom();
      return;
    }
    if (planDirty || !workflowState.teacherConfirmed || !workflowState.cardsGenerated) scrollToSection('teacher-plan-editor');
    else {
      const nextCard = cards.findIndex(card => card.status !== 'locked' || !card.items?.length);
      if (nextCard >= 0) setActiveCard(nextCard);
      scrollToSection('card-workspace');
    }
  };
  ...
  const stageGuide = [
    { reveal: '先写课题与核心问题', prompt: boardCoreQuestion === '核心问题待补充' ? '请先补充本课核心问题，不用通用问题代替。' : `教师追问：${boardCoreQuestion}`, student: '学生回答后，留下一个关键词。', next: '下一步：从原文找出能够支撑回答的词句。' },
  ...
  return <div className="view-stack cards-page">
    <section className="hero compact-hero cards-hero">
      ...
      <div className="hero-actions"><button type="button" className="primary" onClick={continueClassroomFlow} disabled={!draft || cardsBlocked}><PanelTop/>{classroomButtonCopy}</button><button type="button" onClick={exportMd} disabled={!cards.length} title={!cards.length ? '完成三卡生成后即可导出' : undefined}><Download/>导出方案</button><button type="button" onClick={publishAsset} disabled={!draftId || !workflowState.teacherConfirmed || !workflowState.cardsGenerated} title={!workflowState.cardsGenerated ? '确认方案并生成三卡后即可收进资产库' : !workflowState.teacherConfirmed ? '教师确认本版后即可收进资产库' : undefined}><Archive/>收进教研资产库</button><details className="hero-more-tools"><summary><Menu/>更多课堂工具</summary><div className="hero-more-tools-grid">{draftId && <a href={`/alignment/?draftId=${encodeURIComponent(draftId)}`}><Target/>核对课标</a>}{draftId && workflowState.teacherConfirmed && workflowState.cardsGenerated && <a href={`/share/?draftId=${encodeURIComponent(draftId)}`}><Share2/>发布共备快照</a>}{draftId && workflowState.cardsGenerated && <a href={`/slides/?draftId=${encodeURIComponent(draftId)}`}><PanelTop/>生成课堂课件</a>}{draftId && workflowState.cardsGenerated && <a href={`/homework/?draftId=${encodeURIComponent(draftId)}`}><ClipboardCheck/>生成分层作业</a>}{draftId && workflowState.cardsGenerated && <a href={`/pulse/?draftId=${encodeURIComponent(draftId)}`}><Gauge/>课前学情摸底</a>}{draftId && workflowState.cardsGenerated && <a href={`/rehearsal/?draftId=${encodeURIComponent(draftId)}`}><Route/>预演问题链</a>}{draftId && workflowState.cardsGenerated && <a href={`/reflection/?draftId=${encodeURIComponent(draftId)}`}><History/>查看课后复盘</a>}</div></details></div>
    </section>
    ...
    {busy ? <section className="panel cards-loading-skeleton" aria-label="正在读取课堂设计" aria-busy="true"><span className="sr-only">正在读取课堂设计…</span><div className="skeleton-line skeleton-title"/><div className="skeleton-line skeleton-copy"/><div className="skeleton-card-row"><i/><i/><i/></div></section> : showEmpty ? null : <>
      ...
      <section className="cards-workflow-guide panel" aria-label="一课三卡使用步骤">
        <header aria-live="polite"><div><span>本页使用顺序</span><b>第 {workflowGuideStep + 1} 步 · {workflowGuide[workflowGuideStep][0]}</b></div><p><strong>下一步：</strong>{workflowNextCopy}</p>{workflowState.teacherConfirmed && workflowState.cardsGenerated && <button type="button" className="workflow-primary-jump" onClick={() => scrollToSection('card-workspace')}>继续检查三卡 <ArrowRight/></button>}</header>
    ...
    {lockCandidate && <CardLockConfirmation returnFocusRef={lockTriggerRef} card={cards.find(card => card.id === lockCandidate)} dirty={dirty} pending={Boolean(locking)} error={error} onCancel={() => setLockCandidate(null)} onConfirm={() => lock(cards.find(card => card.id === lockCandidate))}/>}
    {classroom && <div className="classroom-overlay"><div className="classroom-session" ref={classroomRef} role="dialog" aria-modal="true" aria-label="课堂共创记录" tabIndex={-1}><button type="button" className="classroom-close" onClick={closeClassroom} disabled={classroomSaving} aria-label="关闭课堂模式"><X/></button><div className="blackboard-shell"><div className="blackboard-topline"><span>课堂共创板书 · 只记录学生真正说出的内容</span><b>第 {revealed} / {stages.length} 步</b></div>{preClassCue && <section className={`classroom-pulse-cue ${preClassCue.level}`}><div><span>课前学情摸底 · {preClassCue.counts.responded} 人完成判断</span><b>{preClassCue.teacherDecision === 'adopt' ? preClassCue.title : '教师决定保持原课堂主线'}</b><p>{preClassCue.teacherDecision === 'adopt' ? preClassCue.openingMove : '摸底结果已保留；课堂仍按教师原定主线开始，必要时再使用下方教材追问。'}</p></div><CardSourceList citations={draft?.citations || []} refs={preClassCue.citationIds} returnTo={`/cards/?draftId=${encodeURIComponent(draftId)}&classroom=1`}/></section>}<section className="classroom-adaptation" aria-label="课堂应变"><header><div><span>课堂应变</span><b>现在课堂进行得怎样？</b></div><small>{classroomReadOnly ? '本节课的应变记录已确认' : '选择现场情况，系统只用已确认的三卡和教材依据调整下一步。'}</small></header><div className="classroom-pace-options">{[['on_track','节奏正常','继续当前步骤'],['time_short','时间不足','保住主问和收束'],['students_stuck','学生卡住','降低一步再回原文'],['ahead','提前完成','在本篇内加深']].map(([value,label,help]) => <button type="button" key={value} className={classroomRun.paceSignal === value ? 'active' : ''} aria-pressed={classroomRun.paceSignal === value} disabled={classroomSaving || classroomReadOnly} onClick={() => markClassroomPace(value)}><span>{label}</span><small>{help}</small></button>)}</div>{classroomAdaptation && <div className="classroom-adaptation-advice" data-signal={classroomAdaptation.signal}><div className="classroom-adaptation-copy"><span>下一步建议</span><h3>{classroomAdaptation.title}</h3><p><b>现在这样做：</b>{classroomAdaptation.primaryAction}</p><p><b>随后这样收：</b>{classroomAdaptation.secondaryAction}</p><small>{classroomAdaptation.note}</small></div><CardSourceList citations={draft?.citations || []} refs={classroomAdaptation.citationIds} returnTo={`/cards/?draftId=${encodeURIComponent(draftId)}&classroom=1`}/></div>}</section>{rehearsalClassroomStep && <section className="classroom-rehearsal-cue"><div><span>课前预演问题</span><b>{rehearsalClassroomStep.question}</b></div><div><span>备用追问</span><p>{rehearsalClassroomStep.branches?.[rehearsalClassroomStep.selectedOutcome] || '根据学生回答决定是否继续追问。'}</p></div><CardSourceList citations={draft?.citations || []} refs={rehearsalClassroomStep.citationIds} returnTo={`/cards/?draftId=${encodeURIComponent(draftId)}&classroom=1`}/></section>}<MindMapBoard title={lessonTitle} coreQuestion={boardCoreQuestion} boardPlan={board?.boardPlan} items={(board && board.items) || []} stage={revealed} classroomRun={classroomRun}/><div className="blackboard-caption"><strong>{stages[revealed - 1]}</strong><span>{stageGuide[revealed - 1].prompt}</span><small>{stageGuide[revealed - 1].student} {stageGuide[revealed - 1].next}</small><small className="classroom-writing-cue"><b>落笔提示：</b>{boardWritingPlan.steps[revealed - 1]?.write.join(' · ') || '本步不预写答案'} · 约 {boardWritingPlan.steps[revealed - 1]?.seconds || 0} 秒</small></div></div><section className="classroom-record-strip" aria-label="课堂现场记录"><div className="classroom-record-copy"><span>本步课堂情况</span><b>{classroomReadOnly ? '这份课堂记录已经由教师确认' : '点一下即可，不需要在课堂上写长文'}</b><small>{classroomSaving ? '正在保存课堂记录…' : classroomNotice || (classroomDirty ? '本次标记暂存在本机，关闭或结束课堂时会保存。' : '课堂记录已保存到当前账号。')}</small>{classroomConflictRun && <div className="classroom-conflict-actions"><button type="button" onClick={() => chooseClassroomRecord(true)}>使用本机记录</button><button type="button" onClick={() => chooseClassroomRecord(false)}>保留账号记录</button></div>}</div><div className="classroom-outcome-grid">{[['reached','学生已经说出'],['needs_followup','还需要追问'],['not_used','本步未展开']].map(([value, label]) => <button type="button" className={currentStageOutcome === value ? 'active' : ''} aria-pressed={currentStageOutcome === value} disabled={classroomSaving || classroomReadOnly} onClick={() => markClassroomStage(value)} key={value}><CheckCircle2/>{label}</button>)}</div><div className="classroom-keyword-box"><label><span>学生关键词（最多 3 个）</span><div><input value={classroomKeyword} maxLength={16} disabled={classroomSaving || classroomReadOnly || classroomRun.keywords.length >= 3} onChange={event => setClassroomKeyword(event.target.value)} onKeyDown={event => { if (event.key === 'Enter' && !event.nativeEvent.isComposing) { event.preventDefault(); addClassroomKeyword(); } }} placeholder={classroomRun.keywords.length >= 3 ? '已记录 3 个关键词' : '不写姓名，只记关键词'}/><button type="button" onClick={addClassroomKeyword} disabled={classroomSaving || classroomReadOnly || !classroomKeyword.trim() || classroomRun.keywords.length >= 3}>记下</button></div></label><div className="classroom-keywords">{classroomRun.keywords.map(item => <span key={item.id}>{item.text}{!classroomReadOnly && <button type="button" onClick={() => removeClassroomKeyword(item.id)} aria-label={`删除关键词${item.text}`}><X/></button>}</span>)}</div></div><section className="classroom-moment-panel" aria-label="课堂随手记"><header><div><span>课堂随手记</span><b>第 {classroomElapsedMinutes()} 分钟 · 一次只记一个可观察事实</b></div><small>{classroomReadOnly ? '本节课堂时间线已确认' : `已记录 ${classroomRun.moments.length} / 24 条；不写学生姓名，不需要课后重新回忆。`}</small></header><div className="classroom-moment-entry"><input value={classroomMoment} maxLength={80} disabled={classroomSaving || classroomReadOnly || classroomRun.moments.length >= 24} onChange={event => setClassroomMoment(event.target.value)} placeholder="可选：补一句学生表现、原话摘要或时间变化"/><div>{[['breakthrough','说通了'],['confusion','共同卡点'],['question','意外好问题'],['timing','时间变化']].map(([type,label]) => <button type="button" key={type} disabled={classroomSaving || classroomReadOnly || classroomRun.moments.length >= 24} onClick={() => recordClassroomMoment(type)} data-type={type}><Plus/>{label}</button>)}</div></div>{classroomRun.moments.length > 0 && <ol className="classroom-moment-timeline">{classroomRun.moments.slice().reverse().slice(0, 8).map(item => <li key={item.id} data-type={item.type}><time>{item.elapsedMinutes}′</time><span><b>{{ breakthrough:'说通了', confusion:'共同卡点', question:'意外好问题', timing:'时间变化' }[item.type]}</b><small>{item.text}</small></span>{!classroomReadOnly && <button type="button" onClick={() => deleteClassroomMoment(item.id)} aria-label={`删除课堂记录：${item.text}`}><X/></button>}</li>)}</ol>}</section></section><footer className="classroom-controls"><button type="button" onClick={() => changeClassroomStage(1)} disabled={revealed === 1 || classroomSaving}>恢复初始</button><button type="button" onClick={() => changeClassroomStage(revealed - 1)} disabled={revealed === 1 || classroomSaving}><ArrowLeft/>上一步</button><span>课堂展开：{stages[revealed - 1]}</span><button type="button" onClick={() => changeClassroomStage(revealed + 1)} disabled={revealed === stages.length || classroomSaving}>{revealed === stages.length ? '已到最后一步' : '下一步'}<ArrowRight/></button><button type="button" onClick={() => saveClassroomRun()} disabled={classroomSaving || classroomReadOnly || !classroomDirty}>{classroomSaving ? '保存中…' : '保存现场记录'}</button><button type="button" onClick={toggleFullscreen}><Maximize2/>全屏投影</button>{!classroomReadOnly && <button type="button" className="primary" onClick={finishClassroom} disabled={classroomSaving}>结束并整理复盘</button>}</footer></div></div>}
  </div>;
}
