...
${sourceNote}`;
  ...
  const classroomReady = workflowState.teacherConfirmed && workflowState.cardsGenerated && allCardsLocked && Boolean(board?.items?.length);
  ...
  const continueClassroomFlow = () => {
    ...
    if (!workflowState.teacherConfirmed || !workflowState.cardsGenerated) scrollToSection('teacher-plan-editor');
    else if (!allCardsLocked) scrollToSection('card-workspace');
    else scrollToSection('board-preview');
  };
