...
${sourceNote}`;
  ...
  const classroomButtonCopy = classroomReady ? classroomCta : !workflowState.cardsGenerated ? '继续：核对并生成三卡' : !allCardsLocked ? '继续：检查并锁定三卡' : '继续：补全板书卡';
  const continueClassroomFlow = () => {
    if (classroomReady) {
      startClassroom();
      return;
    }
    if (!workflowState.cardsGenerated) scrollToSection('teacher-plan-editor');
  ...
  return <div className="view-stack cards-page">
    ...
    {busy ? <section className="panel cards-loading-skeleton" aria-label="正在读取课堂设计" aria-busy="true"><span className="sr-only">正在读取课堂设计…</span><div className="skeleton-line skeleton-title"/><div className="skeleton-line skeleton-copy"/><div className="skeleton-card-row"><i/><i/><i/></div></section> : showEmpty ? null : <>
      ...
      <section className="cards-workflow-guide panel" aria-label="一课三卡使用步骤">
        <header aria-live="polite"><div><span>本页使用顺序</span><b>第 {workflowGuideStep + 1} 步 · {workflowGuide[workflowGuideStep][0]}</b></div><p><strong>下一步：</strong>{workflowNextCopy}</p>{workflowState.cardsGenerated && <button type="button" className="workflow-primary-jump" onClick={() => scrollToSection('card-workspace')}>继续检查三卡 <ArrowRight/></button>}</header>
