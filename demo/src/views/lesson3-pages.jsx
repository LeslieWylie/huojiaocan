...
export function TeachingSlidesPage() {
  ...
  const persist = async confirm => {
    ...
    try {
      ...
      const code = requestCode(err);
      setError(code === 'edit_conflict' ? '这份方案刚刚在其他页面更新。请重新读取后再保存，当前页面内容暂未覆盖服务器版本。' : code === 'teaching_slides_image_invalid' ? '图片没有通过课件保存检查。请删除这张图，重新选择本地 PNG、JPG 或 WebP。' : code === 'teaching_slides_images_too_large' ? '当前课件的图片总量过大。请删减图片后再保存。' : askErrorMessage(err));
    } finally { setWorking(''); }
  };
  ...
  const paginate = async (preview = true) => {
    if (!deck || dirty || working || !draftVersion) return;
    setWorking('paginate'); setError(''); setNotice('');
    try {
      const data = await rootRequest(`/api/drafts/${encodeURIComponent(draftId)}/slides`, { method: 'PATCH', body: { version: draftVersion, paginate: true, preview } });
      setDeck(data.deck); setActive(0);
      if (preview) setPaginationPreview(data.report);
      else {
        setSavedDeck(data.deck); setDraftVersion(Number(data.draftVersion)); setPaginationPreview(null);
        setNotice(`分页修订版已保存，共 ${data.deck.slides.length} 页；原课件已完整保留在修订历史。`);
      }
    } catch (err) { setError(requestCode(err) === 'teaching_slides_page_limit' ? '分页后超过 120 页，未改动原课件。请先拆分内容。' : requestCode(err) === 'edit_conflict' ? '课件已在其他页面更新，请重新读取后再分页；原课件未被覆盖。' : askErrorMessage(err)); }
    finally { setWorking(''); }
  };
