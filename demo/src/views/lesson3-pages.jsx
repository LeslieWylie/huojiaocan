...
export function TeachingSlidesPage() {
  ...
  const [paginationPreview, setPaginationPreview] = useState(null);
  const stageRef = useRef(null);
  const load = () => {
    setBusy(true); setError(''); setNotice(''); setPaginationPreview(null);
    ...
    rootRequest(`/api/drafts/${encodeURIComponent(draftId)}/slides`).then(data => {
      if (data.unavailableReason) {
        setDeck(null); setSavedDeck(null); setDraftVersion(Number(data.draftVersion || 0)); setStale(false); setDirtySlideIds(new Set());
        setError(data.unavailableReason === 'teaching_slides_require_confirmed_plan' ? '请先确认当前教学方案，再生成课堂课件。' : '请先生成一课三卡，再把课堂主线整理成课件。');
        return;
      }
      setDeck(data.deck || null); setSavedDeck(data.deck || null); setDraftVersion(Number(data.draftVersion || 0)); setStale(Boolean(data.stale)); setActive(0); setDirtySlideIds(new Set());
    }).catch(err => {
      const code = requestCode(err);
      setError(code === 'teaching_slides_require_confirmed_plan' ? '请先确认教学方案，再生成课堂课件。' : code === 'teaching_slides_require_cards' ? '请先生成一课三卡，再把课堂主线转成课件。' : code === 'draft_not_found' ? '没有找到这份课堂方案，或它不属于当前账号。' : askErrorMessage(err));
    }).finally(() => setBusy(false));
  };
  ...
  const current = deck?.slides?.[active] || null, readOnly = deck?.status === 'confirmed' || Boolean(paginationPreview), dirty = dirtySlideIds.size > 0;
  const markChanged = (slideId, updater) => {
    if (readOnly) return;
    setDeck(previous => ({ ...previous, slides: previous.slides.map(item => item.id === slideId ? updater(item) : item) }));
    setDirtySlideIds(previous => new Set(previous).add(slideId)); setNotice('');
  };
  const updateCanvas = content => current && markChanged(current.id, item => ({ ...item, content }));
  const updateTeacherNotes = value => current && markChanged(current.id, item => ({ ...item, metadata: { ...item.metadata, teacherNotes: value.split(/\r?\n/u) } }));
  const persist = async confirm => {
    if (!deck || !savedDeck || !draftVersion || readOnly || working || (confirm && dirty)) return;
    setWorking(confirm ? 'confirm' : 'save'); setError(''); setNotice('');
    let serverDeck = savedDeck, version = draftVersion;
    try {
      if (confirm) {
        const data = await rootRequest(`/api/drafts/${encodeURIComponent(draftId)}/slides`, { method: 'PATCH', body: { version, confirm: true } });
        serverDeck = data.deck; version = Number(data.draftVersion || version + 1);
      } else {
        const { createReplaceSlideTransaction } = await import('../teaching-slide-canvas.jsx');
        const { persistLegacyTeachingSlideAssets } = await import('../slide-assets.js');
        const preparedDeck = await persistLegacyTeachingSlideAssets(deck, dirtySlideIds);
        for (const slideId of dirtySlideIds) {
          const before = serverDeck.slides.find(item => item.id === slideId), after = preparedDeck.slides.find(item => item.id === slideId);
          if (!before || !after) continue;
          const data = await rootRequest(`/api/drafts/${encodeURIComponent(draftId)}/slides`, { method: 'PATCH', body: {
            version, slideId, transaction: createReplaceSlideTransaction(before.content, after.content), metadataPatch: { teacherNotes: after.metadata.teacherNotes }
          } });
          serverDeck = data.deck; version = Number(data.draftVersion || version + 1);
        }
      }
      setDeck(serverDeck); setSavedDeck(serverDeck); setDraftVersion(version); setDirtySlideIds(new Set()); setStale(false);
      setNotice(confirm ? '课件已经定稿。投屏文件只包含 OpenMAIC 渲染的学生画布。' : '画布修改已保存；刷新页面后仍可恢复。');
    } catch (err) {
      if (version !== draftVersion) { setSavedDeck(serverDeck); setDraftVersion(version); }
      const code = requestCode(err);
      setError(code === 'edit_conflict' ? '这份方案刚刚在其他页面更新。请重新读取后再保存，当前页面内容暂未覆盖服务器版本。' : code === 'teaching_slides_notes_limit' ? '教师备注超出限制（每页 40 条、单条 8000 字、合计 24000 字）；输入仍保留，请调整后保存。' : code === 'teaching_slides_image_invalid' ? '图片没有通过课件保存检查。请删除这张图，重新选择本地 PNG、JPG 或 WebP。' : code === 'teaching_slides_images_too_large' ? '当前课件的图片总量过大。请删减图片后再保存。' : askErrorMessage(err));
    } finally { setWorking(''); }
  };
  const downloadProjector = async () => {
    if (!deck || working) return;
    setWorking('export'); setError(''); setNotice('');
    try { const { downloadTeachingSlidesProjector } = await import('../teaching-slide-canvas.jsx'); await downloadTeachingSlidesProjector(deck); setNotice('离线投屏稿已按 OpenMAIC 画布导出；文件中不包含教师提示和教师用书答案。'); }
    catch (err) { setError(`投屏稿导出失败：${askErrorMessage(err)}`); }
    finally { setWorking(''); }
  };
  const downloadPptx = async () => {
    if (!deck || working) return;
    setWorking('pptx'); setError(''); setNotice('');
    try { const { downloadTeachingSlidesPptx } = await import('../teaching-slides-pptx.js'); await downloadTeachingSlidesPptx(deck); setNotice('可编辑 PPTX 已生成；文字、图片、表格和线条仍可在 PowerPoint 中修改，公式以可编辑源码文字导出。'); }
    catch (err) { setError(`PPTX 导出失败：${askErrorMessage(err)}`); }
    finally { setWorking(''); }
  };
  const createRevision = async () => {
    if (!deck || !draftVersion || !readOnly || working) return;
    setWorking('revise'); setError(''); setNotice('');
    try {
      const data = await rootRequest(`/api/drafts/${encodeURIComponent(draftId)}/slides`, { method: 'PATCH', body: { version: draftVersion, revise: true } });
      setDeck(data.deck); setSavedDeck(data.deck); setDraftVersion(Number(data.draftVersion || draftVersion + 1)); setDirtySlideIds(new Set());
      setNotice('已从定稿创建新的可编辑修订版；原定稿已保留。');
    } catch (err) { setError(requestCode(err) === 'edit_conflict' ? '这份方案刚刚在其他页面更新，请重新读取后再创建修订版。' : askErrorMessage(err)); }
    finally { setWorking(''); }
  };
  const reviseContent = async (action = 'paginate', preview = true) => {
    if (!deck || dirty || working || !draftVersion) return;
    setWorking(action); setError(''); setNotice('');
    try {
      const data = await rootRequest(`/api/drafts/${encodeURIComponent(draftId)}/slides`, { method: 'PATCH', body: { version: draftVersion, [action]: true, preview } });
      setDeck(data.deck); setActive(action === 'studentQuestions' && data.report.changedPages.length ? data.report.changedPages[0].page - 1 : 0);
      if (preview) setPaginationPreview({ ...data.report, action });
      else {
        setSavedDeck(data.deck); setDraftVersion(Number(data.draftVersion)); setPaginationPreview(null);
        setNotice(`${action === 'studentQuestions' ? '学生问题修订版' : '分页修订版'}已保存，共 ${data.deck.slides.length} 页；原课件完整保留在修订历史。${action === 'studentQuestions' ? `可确认分段的预期回应已移入教师备注。${data.report.protectedPages.length ? `另有 ${data.report.protectedPages.length} 页保持原样，仍需人工确认。` : ''}` : ''}`);
      }
    } catch (err) { setError(requestCode(err) === 'teaching_slides_notes_limit' ? '每页教师备注最多 40 条，单条不超过 8000 字、合计不超过 24000 字；当前内容保留，请调整后重试。' : requestCode(err) === 'teaching_slides_page_limit' ? '分页后超过 120 页，未改动原课件。请先拆分内容。' : requestCode(err) === 'edit_conflict' ? '课件已在其他页面更新，请重新读取后再分页；原课件未被覆盖。' : askErrorMessage(err)); }
    finally { setWorking(''); }
  };
  ...
  return <div className="view-stack teaching-slides-page">
    ...
    {notice && <section className="quality-box"><CheckCircle2/><span>{notice}</span></section>}
    {stale && <section className="slides-stale"><RefreshCw/><div><b>方案或三卡已经更新</b><p>已保存的画布保持原样，不会覆盖教师编辑。分页修订只整理当前画布，不会重新生成方案内容。</p></div></section>}
    {paginationPreview && <section className="slides-stale"><RefreshCw/><div><b>{paginationPreview.action === 'studentQuestions' ? '学生问题修订预览' : '分页预览'}：{paginationPreview.before} → {paginationPreview.after} 页</b><p>当前只预览，尚未保存。{paginationPreview.action === 'studentQuestions' ? '只整理可确认的提问分段：主问、追问留在学生画布，完整预期回应移入教师备注。切换教师备课可核对。' : '正文保持原字号；教师提示和教材引用随续页保留。'}</p>{paginationPreview.action === 'studentQuestions' && <><p>可处理：{paginationPreview.changedPages.length ? paginationPreview.changedPages.map(item => `「${item.title}」${item.responses} 条预期回应`).join('、') : '没有可自动分离的提问组，未改动内容。'}</p>{paginationPreview.imagePages?.length > 0 && <p>图片页保留原排版：{paginationPreview.imagePages.map(item => `第 ${item.page} 页「${item.title}」`).join('、')}。</p>}</>}{paginationPreview.protectedPages.length > 0 && <p>原样保留：{paginationPreview.protectedPages.map(item => `第 ${item.page} 页「${item.title}」`).join('、')}。{paginationPreview.action === 'studentQuestions' ? '这些提问页仍需人工确认，未自动移走任何内容。' : '含图片、自定义版式或复杂格式的页面请手动整理。'}</p>}<button type="button" disabled={Boolean(working) || !paginationPreview.changedPages.length} onClick={() => reviseContent(paginationPreview.action, false)}>{paginationPreview.action === 'studentQuestions' ? '保存学生问题修订版' : '保存分页修订版'}</button><button type="button" disabled={Boolean(working)} onClick={() => { setDeck(savedDeck); setPaginationPreview(null); setActive(0); }}>取消预览</button></div></section>}
    {busy ? <section className="panel study-empty"><Activity/><h2>正在把课堂主线整理成 OpenMAIC 画布</h2><p>只读取当前账号已确认的方案、三卡和真实教材页码。</p></section> : deck && <>
      <section className="slides-toolbar panel"><div><span>查看方式</span><div className="slides-mode-switch"><button type="button" className={mode === 'student' ? 'active' : ''} onClick={() => setMode('student')}><Eye/>学生画布</button><button type="button" className={mode === 'teacher' ? 'active' : ''} onClick={() => setMode('teacher')}><FileCheck2/>教师备课</button></div></div><div className="slides-status"><Badge tone={readOnly ? 'green' : dirty ? 'orange' : 'gold'}>{paginationPreview ? '修订预览 · 未保存' : readOnly ? '课件已定稿' : dirty ? `${dirtySlideIds.size} 页未保存` : '可继续编辑'}</Badge><small>{paginationPreview ? '预览只读；保存修订版后可编辑' : mode === 'student' ? '双击文字编辑，拖动调整版式' : '教师提示只在备课视图显示'}</small></div><div className="slides-save-actions">{!paginationPreview && <><button type="button" title={dirty ? '请先保存画布修改' : '预览分离提问卡中的教师预期回应，原稿保留'} onClick={() => reviseContent('studentQuestions')} disabled={dirty || Boolean(working)}>{working === 'studentQuestions' ? '正在整理…' : '分离教师预期回应'}</button><button type="button" title={dirty ? '请先保存画布修改' : '预览按当前画布全文分页；自定义页面原样保留'} onClick={() => reviseContent('paginate')} disabled={dirty || Boolean(working)}>{working === 'paginate' ? '正在分页…' : '生成分页修订版'}</button></>}{paginationPreview ? null : readOnly ? <button type="button" className="primary" onClick={createRevision} disabled={Boolean(working)}>{working === 'revise' ? '正在创建…' : '创建修订版'}</button> : <><button type="button" onClick={() => persist(false)} disabled={!dirty || Boolean(working)}>{working === 'save' ? '正在保存…' : '保存画布'}</button><button type="button" className="primary" onClick={() => persist(true)} disabled={dirty || Boolean(working)}>{working === 'confirm' ? '正在定稿…' : '确认课件定稿'}</button></>}<button type="button" onClick={downloadPptx} disabled={Boolean(working)}><Download/>可编辑 PPTX</button><button type="button" onClick={downloadProjector} disabled={Boolean(working)}><Download/>投屏稿</button></div></section>
      <section className="slides-workbench openmaic-workbench">
        ...
        <aside className="slides-editor"><header><span>{mode === 'student' ? '学生课件编辑' : '教师备课提示'}</span><h2>第 {active + 1} 页</h2><p>{paginationPreview ? '修订预览只读；保存修订版后可继续编辑。' : readOnly ? '课件已经定稿；画布保持只读。' : mode === 'student' ? '使用画布上方工具插入文字、图片、表格、线条或公式。' : '教师提示与学生画布分开保存。'}</p></header>{mode === 'student' ? <div className="slides-editor-guide"><b>当前开放的元素</b><span>文字 · 图片 · 表格 · 线条 · 公式</span><p>本地图片会自动压缩并随课件保存；音视频、图表和代码暂不开放。</p></div> : <div className="slides-editor-fields"><label><span>教师提示与预期回应（仅备课视图及 PPTX 备注）</span><textarea rows="10" value={(current?.metadata?.teacherNotes || []).join('\n')} disabled={readOnly} onChange={event => updateTeacherNotes(event.target.value)}/><small>{current?.metadata?.teacherNotes?.length || 0} 条教师备注 · 学生投屏不显示</small></label><div className="slides-teacher-sources"><b>教师用书核验页</b>{(current?.metadata?.teacherCitationIds || []).map(refText).filter(Boolean).length ? current.metadata.teacherCitationIds.map(refText).filter(Boolean).map(item => <span key={item}>{item}</span>) : <p>本页没有绑定教师用书页面，不会伪造参考答案。</p>}</div></div>}<footer><ShieldCheck/><p><b>学生投屏隔离</b>学生投屏只显示画布；教师提示与预期回应保留在备课视图和 PPTX 备注中。</p></footer></aside>
      </section>
    </>}
  </div>;
}
