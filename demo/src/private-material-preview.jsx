  ...
  useEffect(() => {
    ...
    if (owner) rootRequest(`/api/materials?documentId=${encodeURIComponent(documentId)}`, { signal: controller.signal })
      .then(data => {
        ...
        setSource(data);
      }).catch(e => { if (active) setError(e.code || e.message || 'original_unavailable'); });
    return () => { active = false; clearTimeout(timeout); controller.abort(); };
  }, [documentId, owner, attempt]);
  const retry = () => { setSource(null); setError(''); setRenderError(false); setAttempt(n => n + 1); };
  if (!owner) return <div className="index-empty"><b>请登录后查看自己的原始材料</b><a href="/login/">登录</a></div>;
  if (error || renderError) return <div className="index-empty" role="alert"><b>{renderError ? '原始 PDF 绘制失败' : error === 'original_not_found' ? '未找到该材料的原始 PDF' : error === 'material_not_found' ? '该材料不存在或不属于当前账号' : '原始 PDF 地址暂时无法读取'}</b><p>{renderError ? '可以重新获取原件地址并加载，不会修改材料。' : '重试只读取原件，不会重新导入或检查材料。'}</p><button type="button" onClick={retry}>重新读取原件</button></div>;
