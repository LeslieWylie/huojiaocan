...
function ingestErrorMessage(code) {
  return ({
    ...
    storage_not_configured: '生产存储尚未配置，原始教材 无法安全保存。',
    storage_configuration_incomplete: '生产存储配置不完整，请联系部署人员。',
    storage_remote_unavailable: '原始教材 存储暂时不可用，请稍后重试。',
    storage_remote_write_failed: '原始教材 保存失败，请稍后重试。',
    stored_but_registration_failed: '原始教材 已保存，但文档登记失败，请稍后重试。',
    ...
    ocr_unavailable: '页面识别服务暂时不可用，原始教材 已保留；请稍后重试。',
    ocr_failed: '页面识别未完成，原始教材 已保留；请检查页面后重试。',
...
function Sidebar({ active, open, close }) {
  ...
  return <>{open && <button className="sidebar-scrim" aria-label="关闭导航" onClick={close}/>}<aside className={`sidebar ${open ? 'open' : ''}`}>
    ...
    <div className="sidebar-foot"><ShieldCheck size={15}/><span>原始教材 是唯一可核验的依据</span></div>
  </aside></>;
}
...
const GUIDANCE_STEPS = [
  ...
  ['回到学生教材', '回到课文原页核对词句、任务和段落结构；原始教材 始终是课堂核验的真源。'],
...
function cardEditGuidance(type) { return CARD_EDIT_GUIDANCE[type] || '把教材依据整理成课堂中可以直接使用的动作，并保留返回原始教材 的依据。'; }
...
${sourceNote}`;
  ...
  return <div className="view-stack cards-page">
    <section className="hero compact-hero cards-hero">
      <div><Badge tone="green"><Layers3/> 一课三卡</Badge><h1>{draft ? lessonTitle : '先选篇目，再生成一课三卡'}</h1><p>{draft ? '先看清这节课的主线，再逐张整理板书、提问和评价；每个关键判断都能回到原始教材 核验。' : '请先在备课问答中选定篇目、形成方案并保存。完成后，这里会出现可编辑的板书卡、提问卡和评价卡。'}</p></div>
    ...
    {busy ? <div className="panel answer-loading"><p>正在读取账号草稿…</p></div> : showEmpty ? null : <>
      ...
      <section className="cards-overview panel">
        ...
        <div className={`curriculum-alignment-entry ${draft?.answer?.curriculumAlignment?.status || 'missing'}`}><Target/><div><b>{draft?.answer?.curriculumAlignment ? '已保存课标对齐' : '还没有确认课标对齐'}</b><p>{draft?.answer?.curriculumAlignment ? '学段要求、任务群候选和学业质量已分开记录，可以随时回到原始教材 核验。' : '先找到课标原页，再由教师决定本课如何对齐学习任务群。'}</p></div>{draftId && <a href={`/alignment/?draftId=${encodeURIComponent(draftId)}`}>{draft?.answer?.curriculumAlignment ? '重新核对' : '开始核对'} <ArrowRight/></a>}</div>
  ...
  return <div className="view-stack worksheet-page">
    ...
    {busy ? <section className="panel worksheet-empty"><Activity/><h2>正在整理本课任务</h2><p>只读取当前账号中的教师定稿、三卡和已核验教材页。</p></section> : !approved ? <section className="panel worksheet-empty"><FileCheck2/><span>生成前</span><h2>请先确认本版教学方案</h2><p>任务单只能来自教师已经确认的方案与三卡，不能把问答阶段的建议直接发给学生。</p><a className="primary" href={`/cards/?draftId=${encodeURIComponent(draftId)}`}>返回确认方案</a></section> : worksheet.status === 'blocked' ? <section className="panel worksheet-empty"><CircleAlert/><span>依据不足</span><h2>三卡还没有绑定学生教材原页</h2><p>学生任务单不能只依据教师用书生成。请回到三卡，为提问与评价补上学生教材页码。</p><a className="primary" href={`/cards/?draftId=${encodeURIComponent(draftId)}`}>返回补充教材依据</a></section> : <>
      ...
      <section className={`worksheet-paper panel ${activePage}`}>
        ...
        <footer><span>{activePage === 'student' ? '学生页不包含教师用书内容或参考提示' : `共使用 ${worksheet.usedCitationCount} 个已核验教材页面`}</span><span>原始教材 是唯一可核验依据</span></footer>
      </section>
    </>}
  </div>;
}
  ...
  if (recipientMode) {
    ...
    return <div className="view-stack share-viewer-page">
      ...
      {snapshot && <>
        ...
        <section className="panel share-evidence"><div><span>可核对的教材页码</span><h2>只分享定位信息，不复制教材原文</h2><p>点击页码可返回活教参的原始教材 核验页。</p></div><div>{snapshot.citations.map(item => { const href = citationLink(item, shareReturnTo); return href ? <a href={href} key={item.id}><FileText/><span><b>{docName(item.documentId)}</b><small>第 {item.pdfPage} 页{item.printedPage ? ` · 书页 ${item.printedPage}` : ''}</small></span><ExternalLink/></a> : null; })}</div></section>
...
function Pitch() { return <div className="view-stack"><section className="hero compact-hero"><div><Badge tone="green"><Play/> 使用示例</Badge><h1>不是普通问答机器人，<br/>而是基于原始教材依据的教学工作台</h1><p>教材全解析、教材目录路由、依据优先问答与课堂逐步展开构成完整流程。</p></div><div className="hero-actions"><a className="primary" href="/ask/?q=《我爱这土地》第二节为什么不能删？"><Play/>进入现场问题</a></div></section></div>; }
...
function ProviderResult({ title, time, tone, question, result, status, providerState }) {
  ...
  return <article className="panel provider-result">
    ...
    {hit ? <>
      ...
      <a className="provider-open" href={openHref}>打开原始教材 核验 <ExternalLink/></a>
...
function DocumentPage() {
  ...
  const downloadTeachingCard = () => {
    ...
    window.setTimeout(() => URL.revokeObjectURL(url), 1000);
    setTeachingCardNotice('讲解卡已下载，原始教材 页码已保留。');
  };
  ...
  return <div className="view-stack document-page">
    ...
    <div className={`verification-workbench${paired ? ' paired-reading-workbench' : ''}`}>
      ...
      {paired ? <section className="paired-pdf-pane">
        ...
        {loading && <div className="evidence-missing"><Activity/><div><b>正在读取页面信息</b><small>左侧原始教材 可继续查看。</small></div></div>}
