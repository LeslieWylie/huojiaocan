...
const ROUTES = [
  ...
  ['document', '/document/', BookOpen, '原始 PDF 核验'],
...
const JOB_STAGES = ['文件检查', '读取 PDF 页面信息', '读取页面文字与页面识别', '整理教材目录与章节', '建立教材目录', '核对搜索与页码', '可用于问答和三卡生成'];
...
async function request(path, options = {}) { return fetchJson(`${API}${path}`, options); }
async function rootRequest(path, options = {}) { return fetchJson(path, options); }
...
function ingestErrorMessage(code) {
  return ({
    ...
    storage_not_configured: '生产存储尚未配置，原始 PDF 无法安全保存。',
    storage_configuration_incomplete: '生产存储配置不完整，请联系部署人员。',
    storage_remote_unavailable: '原始 PDF 存储暂时不可用，请稍后重试。',
    storage_remote_write_failed: '原始 PDF 保存失败，请稍后重试。',
    stored_but_registration_failed: '原始 PDF 已保存，但文档登记失败，请稍后重试。',
    ...
    ocr_unavailable: '页面识别服务暂时不可用，原始 PDF 已保留；请稍后重试。',
    ocr_failed: '页面识别未完成，原始 PDF 已保留；请检查页面后重试。',
    ocr_requires_pdf_ingest: '页面识别需要重新导入原始 PDF，不能复用旧的页面文字快照。',
...
function Sidebar({ active, open, close }) {
  ...
  return <>{open && <button className="sidebar-scrim" aria-label="关闭导航" onClick={close}/>}<aside className={`sidebar ${open ? 'open' : ''}`}>
    ...
    <div className="sidebar-foot"><ShieldCheck size={15}/><span>原始 PDF 是唯一可核验的依据</span></div>
  </aside></>;
}
function Layout({ active, children }) {
  ...
  const aiLabel = aiState === 'ready' ? '系统 AI 可用' : aiState === 'needs-key' ? '需要配置 AI 密钥' : aiState === 'login' ? '需要登录后开始备课' : aiState === 'unavailable' ? 'AI 服务暂时不可用' : '正在检查 AI 服务';
...
const GUIDANCE_STEPS = [
  ['选定篇目', '从教材目录或搜索结果打开课文起始页，先确认物理页、印刷页和章节范围。'],
  ['核对课程标准', '找到学段要求、相关学习任务群和学业质量原页；篇目的具体对齐由教师确认。'],
  ['读教师用书', '优先查看教师用书中的教学目标、重点难点、活动顺序和参考处理。'],
  ['回到学生教材', '回到课文原页核对词句、任务和段落结构；原始 PDF 始终是课堂核验的真源。'],
...
function GuidancePage() {
  return <div className="view-stack guidance-page">
    ...
    <section className="panel guidance-video-panel"><div className="guidance-video-intro"><Badge tone="gold">三分钟看懂</Badge><h2>一条备课路径，五个动作完成</h2><p>建议第一次使用时完整看一遍。以后从教材库进入某篇课文，就可以沿着同样的顺序继续。</p><div className="guidance-video-note"><CheckCircle2/><span><b>视频中的每一步都能在页面中直接完成</b><small>目录定位、原始 PDF 核验、连续追问和课堂材料会沿用同一篇目。</small></span></div></div><div className="guidance-video-frame"><video controls playsInline preload="metadata" poster="/guidance/活教参备课引导封面.svg"><source src="/guidance/活教参备课引导.mp4" type="video/mp4"/><track kind="captions" src="/guidance/活教参备课引导.vtt" srcLang="zh-CN" label="中文字幕" default/>当前浏览器无法播放视频。</video><p>如果视频无法播放，可以直接查看下方的文字步骤。</p></div></section>
...
const CARD_GENERATION_STEPS = [
  '读取教师定稿与教材依据',
  '整理板书、提问和评价初稿',
  '按课堂节奏与原文依据逐项审校',
  '绑定原始页码并保存到当前方案'
];
function lessonTitleFrom(value, fallback = '') {
...
function planIdentity(question, fallback = '') {
  const title = lessonTitleFrom(question, fallback);
  return title || '当前篇目';
}
function lessonRefFromUrl(params, fallbackScope = 'both') {
...
function normalizeCards(source, citations = [], coreQuestion = '', previousCards = []) {
...
function makeBoardPlan(items = [], coreQuestion = '') {
...
function boardLabelFromText(value, fallback = '') {
...
function boardQuestion(value, title) {
...
function withBoardPlan(cards = [], question = '') {
...
function uniqueCitations(citations = [], refs = []) {
...
function normalizeFeedbackForm(value = {}) {
...
function feedbackAdviceFromForm(form = {}) {
  const result = [];
...
function feedbackStorageValue(form = {}) {
...
function cardEditGuidance(type) { return CARD_EDIT_GUIDANCE[type] || '把教材依据整理成课堂中可以直接使用的动作，并保留返回原始 PDF 的依据。'; }
function cardItemNeedsDetail(type, text) {
...
function wrapSvgText(value, max = 13) {
  const text = String(value || '').trim();
  if (!text) return ['待补写'];
...
function CardSourceList({ citations = [], refs = [], returnTo = 'cards' }) {
  ...
  const chip = item => { const href = citationLink(item, returnTo); return href ? <a href={href} key={String(item.documentId) + '-' + citationPage(item)}><Quote size={12}/>{docName(item.documentId)} · PDF {citationPage(item)}页</a> : null; };
...
function Cards() {
  ...
  const exportMd = () => {
    ...
    const refsText = refs => uniqueCitations(draft?.citations || [], refs).map(item => `${docName(item.documentId)} PDF 第${citationPage(item)}页`).join('；');
    ...
    const citations = uniqueCitations(draft?.citations || []).map(item => `- ${docName(item.documentId)} · PDF 第 ${citationPage(item)} 页${item.printedPage ? `（印刷页 ${item.printedPage}）` : ''}`).join('\n');
    const sourceNote = coverage ? `\n\n## 材料覆盖\n${sourceCoverageLabel(coverage)}。缺少的材料不会被当作已引用；正式使用前请教师回看原始 PDF。\n` : '';
...
${sourceNote}`;
  ...
  const classroomReady = workflowState.cardsGenerated && Boolean(board?.items?.length);
  const planFormReady = Boolean(planForm.title.trim() && planForm.summary.trim() && planForm.objectives.trim() && planForm.keyPoints.trim());
  const noticeTitle = ['auth_required','auth_invalid'].includes(errorCode) ? '登录后继续编辑课堂设计' : errorCode === 'draft_missing' ? '还没有选定备课方案' : errorCode === 'draft_not_found' ? '这份备课方案暂时无法读取' : /^(gateway|deepseek|card_generation|evidence_)/u.test(errorCode) ? '三卡暂时没有生成' : '课堂设计暂时没有打开';
  const noticeBody = ['auth_required','auth_invalid'].includes(errorCode) ? '为保护不同账号的课堂资料，只有确认当前账号后才会读取该账号自己的本机恢复副本。' : errorCode === 'draft_missing' ? '先在备课问答中提出问题，保存方案后再进入这里。' : errorCode === 'draft_not_found' ? '可能是链接已过期，或这份方案不属于当前账号。请回到备课问答重新建立方案。' : error || '请稍后重试，或回到备课问答重新建立方案。';
  ...
  return <div className="view-stack cards-page">
    <section className="hero compact-hero cards-hero">
      <div><Badge tone="green"><Layers3/> 一课三卡</Badge><h1>{draft ? lessonTitle : '先选篇目，再生成一课三卡'}</h1><p>{draft ? '先看清这节课的主线，再逐张整理板书、提问和评价；每个关键判断都能回到原始 PDF 核验。' : '请先在备课问答中选定篇目、形成方案并保存。完成后，这里会出现可编辑的板书卡、提问卡和评价卡。'}</p></div>
      <div className="hero-actions"><button type="button" onClick={exportMd} disabled={!cards.length}><Download/>导出方案</button>{draftId && <a href={`/alignment/?draftId=${encodeURIComponent(draftId)}`}><Target/>核对课标</a>}<button type="button" onClick={publishAsset} disabled={!draftId || !workflowState.teacherConfirmed || !workflowState.cardsGenerated}>收进教研资产库</button>{draftId && workflowState.teacherConfirmed && workflowState.cardsGenerated && <a href={`/share/?draftId=${encodeURIComponent(draftId)}`}><Share2/>发布共备快照</a>}{draftId && workflowState.cardsGenerated && <a href={`/slides/?draftId=${encodeURIComponent(draftId)}`}><PanelTop/>生成课堂课件</a>}{draftId && workflowState.cardsGenerated && <a href={`/homework/?draftId=${encodeURIComponent(draftId)}`}><ClipboardCheck/>生成分层作业</a>}{draftId && workflowState.cardsGenerated && <a href={`/pulse/?draftId=${encodeURIComponent(draftId)}`}><Gauge/>课前学情摸底</a>}{draftId && workflowState.cardsGenerated && <a href={`/rehearsal/?draftId=${encodeURIComponent(draftId)}`}><Route/>预演问题链</a>}{draftId && workflowState.cardsGenerated && <a href={`/reflection/?draftId=${encodeURIComponent(draftId)}`}>查看课后复盘</a>}<button type="button" className="primary" onClick={startClassroom} disabled={!classroomReady}><PanelTop/>{classroomReady ? classroomCta : '请先生成板书卡'}</button></div>
    </section>
    ...
    {busy ? <div className="panel answer-loading"><p>正在读取账号草稿…</p></div> : showEmpty ? null : <>
      ...
      <section className="cards-overview panel">
        ...
        <div className={`curriculum-alignment-entry ${draft?.answer?.curriculumAlignment?.status || 'missing'}`}><Target/><div><b>{draft?.answer?.curriculumAlignment ? '已保存课标对齐' : '还没有确认课标对齐'}</b><p>{draft?.answer?.curriculumAlignment ? '学段要求、任务群候选和学业质量已分开记录，可以随时回到原始 PDF 核验。' : '先找到课标原页，再由教师决定本课如何对齐学习任务群。'}</p></div>{draftId && <a href={`/alignment/?draftId=${encodeURIComponent(draftId)}`}>{draft?.answer?.curriculumAlignment ? '重新核对' : '开始核对'} <ArrowRight/></a>}</div>
      ...
      <section className="card-workspace panel">
        <header className="card-workspace-head"><div><span>课堂产物</span><h2>三张卡，分别对应课堂中的三个动作</h2><p>先选一张卡作为主编辑区；每条内容都可以修改、保存、锁定，并从依据芯片回到真实 PDF 页面。</p></div><Badge tone="gold">{currentCard?.status === 'locked' ? '当前卡已锁定' : workflowCopy}</Badge></header>
  ...
  return <div className="view-stack worksheet-page">
    ...
    {busy ? <section className="panel worksheet-empty"><Activity/><h2>正在整理本课任务</h2><p>只读取当前账号中的教师定稿、三卡和已核验教材页。</p></section> : !approved ? <section className="panel worksheet-empty"><FileCheck2/><span>生成前</span><h2>请先确认本版教学方案</h2><p>任务单只能来自教师已经确认的方案与三卡，不能把问答阶段的建议直接发给学生。</p><a className="primary" href={`/cards/?draftId=${encodeURIComponent(draftId)}`}>返回确认方案</a></section> : worksheet.status === 'blocked' ? <section className="panel worksheet-empty"><CircleAlert/><span>依据不足</span><h2>三卡还没有绑定学生教材原页</h2><p>学生任务单不能只依据教师用书生成。请回到三卡，为提问与评价补上学生教材页码。</p><a className="primary" href={`/cards/?draftId=${encodeURIComponent(draftId)}`}>返回补充教材依据</a></section> : <>
      ...
      <section className={`worksheet-paper panel ${activePage}`}>
        ...
        <footer><span>{activePage === 'student' ? '学生页不包含教师用书内容或参考提示' : `共使用 ${worksheet.usedCitationCount} 个已核验教材页面`}</span><span>原始 PDF 是唯一可核验依据</span></footer>
      </section>
    </>}
  </div>;
}
...
function TeachingSlidesPage() {
  ...
  const refText = id => { const item = references.get(String(id)); return item ? `${docName(item.documentId)} PDF 第 ${item.pdfPage} 页` : ''; };
...
function TeachingSharePage() {
  ...
  if (recipientMode) {
    ...
    return <div className="view-stack share-viewer-page">
      ...
      {snapshot && <>
        ...
        <section className="share-cards"><header><span>一课三卡</span><h2>从方案到课堂动作</h2><p>这些内容来自发布时的固定快照，不会跟随原方案后续变动。</p></header><div>{snapshot.cards.map(card => <article className={`share-card share-card-${card.type}`} key={card.id}><header><span>{card.type === 'board' ? '板书卡' : card.type === 'question' ? '提问卡' : card.type === 'assessment' ? '评价卡' : '课堂卡'}</span><h3>{card.title}</h3><p>{card.subtitle}</p></header><ol>{card.items.map((item, index) => <li key={item.id}><span>{String(index + 1).padStart(2, '0')}</span><div><p>{item.text}</p><footer>{item.citationIds.map(id => { const ref = reference(id); const href = ref ? citationLink(ref, shareReturnTo) : ''; return href ? <a key={id} href={href}><Quote/>{docName(ref.documentId)} PDF 第 {ref.pdfPage} 页</a> : null; })}</footer></div></li>)}</ol></article>)}</div></section>
        <section className="panel share-evidence"><div><span>可核对的教材页码</span><h2>只分享定位信息，不复制教材原文</h2><p>点击页码可返回活教参的原始 PDF 核验页。</p></div><div>{snapshot.citations.map(item => { const href = citationLink(item, shareReturnTo); return href ? <a href={href} key={item.id}><FileText/><span><b>{docName(item.documentId)}</b><small>PDF 第 {item.pdfPage} 页{item.printedPage ? ` · 印刷页 ${item.printedPage}` : ''}</small></span><ExternalLink/></a> : null; })}</div></section>
  ...
  return <div className="view-stack share-owner-page">
    ...
    {busy ? <section className="panel share-loading"><Activity/><h2>正在读取当前方案</h2><p>会先确认方案状态，再决定是否允许发布。</p></section> : draft && <div className="share-owner-layout">
      <section className="panel share-publish-sheet"><header><div><span>要发布的方案</span><h2>{draft.title || '未命名备课'}</h2><p>当前第 {draft.version || 1} 版 · {(draft.cards || []).length} 张课堂卡 · {(draft.citations || []).length} 条教材依据</p></div><Badge tone={confirmed ? 'green' : 'orange'}>{confirmed ? '教师已确认' : '还不能发布'}</Badge></header><div className="share-safety-list">{[['独立快照','原方案后续修改，不影响已发布内容'],['最小信息','不包含账号、对话、密钥和私人文档原文'],['可随时撤销','每条链接独立管理，不影响其他分享']].map(([title, note]) => <div key={title}><ShieldCheck/><span><b>{title}</b><small>{note}</small></span></div>)}</div><footer><label><span>链接有效期</span><select value={expiryDays} onChange={event => setExpiryDays(Number(event.target.value))}><option value="7">7 天</option><option value="14">14 天</option><option value="30">30 天</option></select></label><a href={`/cards/?draftId=${encodeURIComponent(draftId)}`}>返回检查方案</a><button type="button" className="primary" onClick={create} disabled={!confirmed || !(draft.cards || []).length || Boolean(working)}>{working === 'create' ? '正在生成快照…' : '生成共备链接'}</button></footer>{!confirmed && <div className="share-blocked-note"><CircleAlert/><p>请先回到课堂设计确认当前版本。未确认修改不会被包装成可分享成果。</p></div>}</section>
...
function Pitch() { return <div className="view-stack"><section className="hero compact-hero"><div><Badge tone="green"><Play/> 使用示例</Badge><h1>不是普通问答机器人，<br/>而是基于原始教材依据的教学工作台</h1><p>PDF 全解析、教材目录路由、依据优先问答与课堂逐步展开构成完整闭环。</p></div><div className="hero-actions"><a className="primary" href="/ask/?q=《我爱这土地》第二节为什么不能删？"><Play/>进入现场问题</a></div></section></div>; }
  ...
  return (
    <div className="view-stack index-page">
      <section className="hero index-hero">
        <div><Badge tone="green"><Library/> 教材库</Badge><h1>先选定要查的材料，<br/>再从目录进入具体篇目</h1><p>课程标准说明学段要求与学业质量，学生教材用于核对课文原页，教师教学用书用于参考课时、活动和教学处理。目录、搜索和 PDF 核验会始终同步。</p></div>
        <div className="index-health"><b>{docs.reduce((sum, item) => sum + item.pageCount, 0)}</b><span>页可定位</span><small>{docs.length} 份材料 · 状态来自服务端</small></div>
      </section>
...
function CitationChips({ citations, refs, returnTo = 'ask', limit = 4 }) {
  ...
  return <div className="citation-chips">{items.slice(0, limit).map(item => { const href = citationLink(item, returnTo); return href ? <a href={href} key={`${item.documentId}-${citationPage(item)}`} title={`${item.documentTitle} · ${item.sectionPath?.join(' › ') || ''}`}><Quote size={11}/>{docName(item.documentId)} · PDF {citationPage(item)}页</a> : null; })}</div>;
}
...
function ConversationSide({
  ...
  return <aside className="panel ask-side conversation-side">
    ...
    {recentDrafts.length ? <details className="conversation-secondary compact-history"><summary>其他备课记录（{recentDrafts.length}）</summary><section className="conversation-drafts">{visibleDrafts.map(item => <a className={draftId && String(item.id) === String(draftId) ? 'current' : ''} href={`/ask/?draftId=${encodeURIComponent(item.id)}`} key={item.id}><span><History size={14}/></span><div><b>{item.title || item.question || '未命名备课'}</b><small>{item.updated_at ? new Date(item.updated_at).toLocaleDateString() : '最近保存'} · 恢复完整对话</small></div><ArrowRight size={14}/></a>)}</section></details> : null}
    {visibleLocalSessions.length ? <details className="conversation-secondary"><summary>本机恢复点（{visibleLocalSessions.length}）</summary><section className="conversation-drafts conversation-local-sessions">{visibleLocalSessions.map(item => <a href={`/ask/?resume=${encodeURIComponent(item.resumeId)}`} key={item.resumeId}><span><History size={14}/></span><div><b>{item.lessonRef?.title || planIdentity(item.planQuestion || item.question, '未命名备课')}</b><small>{item.savedAt ? new Date(item.savedAt).toLocaleString() : '最近保存'} · 继续追问</small></div><ArrowRight size={14}/></a>)}</section></details> : null}
...
function AskPage() {
  ...
  const ask = async (event, directQuestion, options = {}) => {
    ...
    try {
      ...
      const draftPayload = { title: lessonTitle, question: nextIdentityQuestion, scope: scopeDocumentIds(selectedScope), lessonContext: { ...nextLessonContext, ...(nextLessonRef ? { lessonRef: nextLessonRef } : {}) }, answer: { ...(response.answer || {}), sourceCoverage: response.sourceCoverage || response.answer?.sourceCoverage, conversationHistory: nextHistory, conversationTurns: nextConversationTurns, evidenceShelf, ...(sameLesson && existingDraft?.answer?.previousLessonReflection ? { previousLessonReflection: existingDraft.answer.previousLessonReflection } : {}), ...(sameLesson && existingDraft?.answer?.lessonReflection ? { lessonReflection: existingDraft.answer.lessonReflection } : {}) }, citations: nextCitations, cards: sameLesson ? cardsForAskDraft(existingDraft) : [] };
  ...
  const exportConversation = () => {
    ...
    messages.forEach((turn, index) => {
      ...
      if (citations.length) lines.push('', '**教材依据**', ...citations.map(item => `- ${docName(item.documentId)} · PDF 第${item.pdfPage}页：${citationText(item)}`));
    });
    if (evidenceShelf.length) lines.push('', '## 本课依据夹', ...evidenceShelf.map(item => `- ${docName(item.documentId)} · PDF 第${item.pdfPage}页${item.printedPage ? `（印刷页 ${item.printedPage}）` : ''}`));
    ...
    <div className="view-stack ask-page">
      ...
      <section className="panel lesson-context">
        ...
        <div className="context-controls">
          ...
          <ContextSelect label="AI 来源" value={keyId} onChange={e => setKeyId(e.target.value)} options={[{value:'', label: gatewayAvailable ? UI_COPY.provider.systemGateway : '暂无可用 AI'}, ...keys.map(key => ({value:key.id, label:`${key.model} · ${key.keyHint}`}))]} hint={!aiReady ? '正在检查 AI 服务' : keyId ? '个人密钥' : gatewayAvailable ? '系统默认' : '请稍后重试'}/>
        </div>
      ...
      <div className="ask-layout">
        <section className="panel ask-main">
          ...
          {session && !canAsk && aiReady && <div className="ask-auth-note"><CircleAlert/><span>当前没有可用的 AI 连接。可以先在 AI 设置中添加或测试密钥。</span><a href="/settings/">打开 AI 设置</a></div>}
...
function IngestPage() {
  ...
  return <div className="view-stack"><section className="hero compact-hero"><div><Badge tone="green"><Upload/> PDF 导入向导</Badge><h1>原始文件不可变保存，<br/>页面内容按 PDF 物理页逐页处理</h1><p>扫描 PDF、页面文字 PDF 和混合 PDF 都可导入；公开界面始终展示原始 PDF 页面。</p></div><div className="wizard-steps">{['选择文件','文档预检','确认策略','创建任务'].map((x,i)=><span className={step>i?'active':''} key={x}><b>{i+1}</b>{x}</span>)}</div></section>
    {error&&<div className="ask-error"><CircleAlert/>{error}</div>}
    {step===1?<section className="panel upload-panel"><label className="dropzone"><Upload/><h2>拖入 PDF，或点击选择文件</h2><p>支持页面文字、扫描页与混合文档。原始 PDF 不会被解析结果覆盖。</p><input type="file" accept="application/pdf" onChange={e=>setFile(e.target.files?.[0]||null)}/><span>{file?file.name:'选择 PDF 文件'}</span></label><button className="primary wide-action" onClick={preflight} disabled={!file}>检查当前文件</button></section>:<><section className="panel"><SectionHead icon={FileCheck2} eyebrow="文档预检" title={displayFile.name} note="当前页面先检查文件；提交后会逐页读取内容并建立目录。"/><div className="preflight-grid"><Stat icon={FileText} label="文件大小" value={`${(displayFile.size/1024/1024).toFixed(1)} MB`} note="登记后由服务端计算 SHA-256"/><Stat icon={BookOpen} label="预计页数" value="提交后读取" note="物理页码由解析任务确认"/><Stat icon={FileSearch} label="页面文字" value="逐页检查" note="质量合格时直接使用" tone="green"/><Stat icon={Eye} label="扫描页面" value="自动判断" note="只识别缺失或质量不足的页面" tone="gold"/></div><div className="preview-strip">{['起始页','中段页','末页'].map(label=><div key={label}><div className="preview-paper"><FileText/><span>原始 PDF<br/>{label}</span></div><b>任务完成后可核验</b></div>)}</div></section><section className="panel confirm-grid"><div><label>文档分类</label><select value={kind} onChange={e=>setKind(e.target.value)}><option value="textbook">学生教材</option><option value="teacher_guide">教师教学用书</option><option value="other">其他教学资料</option></select></div><div><label>提取策略</label><select value={policy} onChange={e=>setPolicy(e.target.value)}><option value="auto">自动判断（推荐）</option><option value="native">仅使用页面文字</option><option value="ocr">强制重新识别页面</option></select></div><div className="policy-note"><CheckCircle2/><span><b>物理页码保护</b><small>任何解析和人工修正都不会改变 PDF 第几页。</small></span></div><button className="primary" onClick={createTask} disabled={working}>{working?'正在登记并创建任务…':'确认并开始处理'} <ArrowRight/></button>{created&&<small>已开始处理，正在进入进度页面。</small>}</section></>}
  </div>;
}
...
function InspectPage() {
  ...
  return <div className="view-stack"><section className="panel inspect-toolbar"><div><Badge tone={info.tone}>{info.short}</Badge><h1>页面校正</h1><p>原始 PDF 不可修改；这里只调整用于搜索的页面文字、标题、章节和印刷页码。</p></div><div><button onClick={()=>setPage(p=>Math.max(1,p-1))}>上一页</button><input value={page} onChange={e=>setPage(Math.max(1,Number(e.target.value)||1))}/><button onClick={()=>setPage(p=>p+1)}>下一页</button><a href={`/document/?doc=${encodeURIComponent(doc)}&page=${page}`}>核验原页 <ExternalLink/></a></div></section>{error&&<div className="ask-error"><CircleAlert/>{error}</div>}{notice&&<div className="quality-box"><CheckCircle2/>{notice}</div>}<div className="inspect-layout"><section className="panel original-preview"><header><b>原始 PDF · 物理页 {page}</b><Badge tone="green">唯一可核验的依据</Badge></header>{rawPdfUrl?<iframe key={`${doc}-${page}`} title="原始 PDF 页面" src={pdfPageUrl(rawPdfUrl,page)}/>:<div className="index-empty"><FileText/><b>正在读取原始 PDF</b><p>页面信息加载后会在这里显示对应原页。</p></div>}</section><section className="panel extraction-editor"><SectionHead icon={FileSearch} eyebrow="用于教材搜索的文字" title={loading?'正在读取页面…':'当前生效文字'}/><div className="source-tabs"><button className={source==='native'?'active':''} onClick={()=>setSource('native')}>页面文字</button><button className={source==='ocr'?'active':''} onClick={()=>setSource('ocr')}>扫描页文字</button><button className={source==='retrieval'?'active':''} onClick={()=>setSource('retrieval')}>当前生效文字</button></div><p className="inspect-source-note">{ocrNote}</p><textarea value={shownText} readOnly={source!=='retrieval'} onChange={e=>setRetrievalText(e.target.value)} placeholder={loading?'正在加载…':source==='ocr'&&!ocrText?'本页没有页面识别文字':'该文本来源暂无内容'}/><div className="editor-fields"><label>页面标题<input value={title} onChange={e=>setTitle(e.target.value)}/></label><label>印刷页码<input value={printedPage} onChange={e=>setPrintedPage(e.target.value)}/></label><label>章节路径<input value={sectionPath} onChange={e=>setSectionPath(e.target.value)}/></label></div><div className="quality-box"><CheckCircle2/><span><b>质量状态：{record?.textQualityStatus||record?.qualityStatus||'待读取'}</b><small>页面文字来源：{sourceName} · PDF 物理页 {page} 保持不变</small></span></div><div className="editor-actions"><label><input type="checkbox" checked={included} onChange={e=>setIncluded(e.target.checked)}/>纳入教材搜索</label><button onClick={rerun} disabled={rerunning||loading}><RefreshCw/>{rerunning?'正在创建任务':'重新读取当前页'}</button><button className="primary" onClick={save} disabled={saving||loading}>{saving?'正在保存':'保存页面调整'}</button></div></section></div></div>;
}
  ...
  return <div className="view-stack">
    <section className="hero compact-hero">
      <div>
        ...
        <p>逐题检查教材目录的真实定位结果、物理页码和引用片段；检查未通过的页面不会被当作可靠依据。</p>
  ...
  return <article className="panel provider-result">
    ...
    {hit ? <>
      <div className="provider-hit-meta">
        ...
        <span><small>PDF 物理页</small><b>{hit.pdfPage}</b></span>
        <span><small>印刷页</small><b>{hit.printedPage || '未标注'}</b></span>
        <span><small>章节路径</small><b>{section}</b></span>
        <span><small>定位状态</small><b>已命中原始页</b></span>
      </div>
      <blockquote>{hit.text || '暂时没有可展示的页面片段。'}</blockquote>
      <a className="provider-open" href={openHref}>打开原始 PDF 核验 <ExternalLink/></a>
...
function DocumentPage() {
  ...
  const title = record?.pageTitle || record?.title || `PDF 第 ${physicalPage} 页`;
  ...
  const downloadTeachingCard = () => {
    ...
    window.setTimeout(() => URL.revokeObjectURL(url), 1000);
    setTeachingCardNotice('讲解卡已下载，原始 PDF 页码已保留。');
  };
  ...
  return <div className="view-stack document-page">
    ...
    {paired && <section className="paired-focus-panel">
      <div className="paired-focus-copy"><span>句段追踪</span><h2>在这一句停住，直接寻找教师用书的相关建议</h2><p>输入课文中的句子、关键词或课后题。篇目身份保持不变，右侧只重新定位对应材料的真实 PDF 页面。</p></div>
    ...
    <div className={`verification-workbench${paired ? ' paired-reading-workbench' : ''}`}>
      ...
      {paired ? <section className="paired-pdf-pane">
        ...
        {!loading && error && <div className="evidence-missing"><CircleAlert/><div><b>本页暂时没有可用教材依据</b><small>{error}。左侧仍显示原始 PDF。</small></div></div>}
        {!loading && !error && tab === 'evidence' && <><Badge tone={info?.tone || 'green'}>{info?.short || '教材'}依据</Badge><h2>{title}</h2>{retrievalText ? <details open><summary>查看原文片段</summary><blockquote>{retrievalText}</blockquote></details> : <div className="evidence-missing"><CircleAlert/><div><b>暂无可复制片段</b><small>请以左侧原始页面为准。</small></div></div>}<div className="source-meta"><span>PDF 物理页</span><b>{physicalPage}</b><span>印刷页</span><b>{printedPage}</b><span>文本来源</span><b>{sourceLabel}</b><span>质量</span><b>{qualityLabel}</b></div></>}
        {!loading && !error && tab === 'context' && <div className="related-citations"><b>相邻原页</b><p>相邻页面仅供核验，不会自动纳入本次依据。</p>{[page - 1, page + 1].filter(value => value >= 1 && value <= maxPage).map(value => <button onClick={() => goto(value)} key={value}><b>打开 PDF 第 {value} 页</b></button>)}</div>}
        {!loading && !error && tab === 'text' && <textarea readOnly value={retrievalText || '本页暂无可复制解析文本。'}/>}<div className="document-switch"><b>关联材料</b><span>点击“打开双源对照”，在同一屏核对两份原始 PDF。</span></div>
      </aside>}
    </div>
    {paired && <section className="panel paired-reading-summary"><article><span>学生此刻看到什么</span><b>{doc === 'textbook' ? title : pairedResult?.title || lessonQuery}</b><p>{doc === 'textbook' ? retrievalText || '请直接核对左侧学生教材原页。' : citationText(pairedResult) || '请直接核对右侧学生教材原页。'}</p></article><i/><article><span>教师此刻参考什么</span><b>{doc === 'teacher-guide' ? title : pairedResult?.title || lessonQuery}</b><p>{doc === 'teacher-guide' ? retrievalText || '请直接核对左侧教师用书原页。' : citationText(pairedResult) || '请直接核对右侧教师用书原页。'}</p></article><small>以上文字只帮助辨认页面；备课结论仍需回到两侧原始 PDF 核验。</small></section>}
    {dualSourceTeachingCard && <section className="panel dual-source-teaching-card">
      ...
      <div className="dual-source-card-columns">
        {[['学生先读什么', dualSourceTeachingCard.textbook], ['教师再参考什么', dualSourceTeachingCard.teacherGuide]].map(([label, source]) => <article key={source.documentId}><div><span>{label}</span><b>{docName(source.documentId)} · PDF 第 {source.pdfPage} 页{source.printedPage ? ` · 印刷页 ${source.printedPage}` : ''}</b><small>{source.section || source.title || '当前篇目'}</small></div><blockquote>{source.excerpt || '当前页面没有可复制片段，请直接打开原始 PDF 核验。'}</blockquote><a href={buildReaderHref({ documentId: source.documentId, page: source.pdfPage, lessonTitle: lessonQuery, focus: pairedFocus, paired: true, returnTo: `${location.pathname}${location.search}` })}>打开这份原始页面 <ExternalLink/></a></article>)}
      </div>
...
function LoginPage({ callback: initialCallback = null }) {
  const params = useMemo(() => queryParams(), []);
...
function SettingsPage() {
  ...
  return <div className="view-stack settings-page"><section className="hero compact-hero"><div><Badge tone="blue"><ShieldCheck/> AI 设置</Badge><h1>配置你的 AI 连接<br/><em>再开始生成备课方案</em></h1><p>系统 AI 默认使用已配置的系统网关；你也可以添加自己的 DeepSeek 密钥。页面只显示末四位和测试状态，完整密钥不会返回浏览器。</p></div></section><section className="panel"><SectionHead icon={Plus} eyebrow="添加个人密钥" title="配置 DeepSeek 官方接口" note="仅支持 deepseek-v4-flash 与 deepseek-v4-pro。这里保存的是你的个人密钥，不要粘贴系统配置密钥。"/><form onSubmit={add} className="key-form"><label>模型<select value={model} onChange={event => setModel(event.target.value)}><option value="deepseek-v4-flash">deepseek-v4-flash</option><option value="deepseek-v4-pro">deepseek-v4-pro</option></select></label><label>DeepSeek 密钥<input type="password" value={apiKey} onChange={event => setApiKey(event.target.value)} placeholder="sk-…" autoComplete="off"/></label><button className="primary" disabled={busy || !apiKey.trim()}>添加并保存</button></form>{error && <div className="ask-error"><CircleAlert/>{error}</div>}{message && <div className="quality-box"><CheckCircle2/>{message}</div>}</section><section className="panel"><SectionHead icon={ShieldCheck} eyebrow="已保存的个人密钥" title="切换与测试" note="完整密钥不会返回浏览器。备课问答中选择“系统网关（默认）”即可使用系统 AI。"/><div className="key-list">{keys.length ? keys.map(item => <article className="key-row" key={item.id}><div><b>{item.keyHint || 'DeepSeek 密钥'}</b><small>{item.model} · {item.lastTestStatus === 'valid' ? '测试通过' : item.lastTestStatus === 'invalid' ? '测试失败' : '尚未测试'}</small></div><Badge tone={item.isActive ? 'green' : 'neutral'}>{item.isActive ? '当前使用' : '未启用'}</Badge><button disabled={busy} onClick={() => action(item.id, 'test')}>测试</button>{!item.isActive && <button disabled={busy} onClick={() => action(item.id, 'activate')}>设为当前</button>}<button disabled={busy} onClick={() => remove(item.id)}>删除</button></article>) : <p className="muted">还没有保存个人密钥。系统 AI 可以直接使用；添加个人密钥后，也可以在问答页切换。</p>}</div></section><div className="hero-actions settings-return"><a className="primary" href="/ask/"><MessageCircle/>返回备课问答</a><a href="/"><Route/>返回教学任务</a></div></div>;
}
