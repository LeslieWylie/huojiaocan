...
2. **备课问答**：固定篇目身份与备课条件，保存历史消息，由 pi-agent-core 运行有限步数的教材搜索工具循环，支持连续追问和教材审校。
...
一次备课请求不是一次模型调用，而是一条有上限、可回退的服务端工作流：

1. 先由 pi-agent-core 检查当前教材页是否足够；模型只能调用 `search_teaching_material`，不足时最多追加两次短查询，不做无目的循环。
2. 在固定篇目、备课条件和服务端教材依据下形成结构化初稿。
3. 第二轮按教材依据和课堂可执行性审校；只有仍存在明确问题时才进入第三轮定向修订。
4. 每轮都运行确定性质量检查。审校结果若比上一版缺少更多课堂结构，服务端保留较好的旧版本。
5. 重试、检索和审校共用一个截止时间；旧对话按完整问答轮次裁剪，当前问题不会被历史长方案挤出上下文。
6. PageIndex 仍是主检索器；Pi 只能提出查询文本。文档 ID、检索范围、物理页、引用文字和 PDF 链接由服务端从 PageIndex 结果与已核验快照重建，不接受模型填写。

检索循环直接复用 `@earendil-works/pi-agent-core`，不再维护一套自研通用 Agent 循环。OpenCode 作为完整的编码 Agent，其文件、终端和代码操作边界与教师问答不匹配，因此不嵌入产品请求。结构化初稿和课堂审校继续使用现有 DeepSeek/系统网关适配器。
