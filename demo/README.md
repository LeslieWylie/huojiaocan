...
1. 先检查当前教材页是否足够；不足时最多追加两次短查询，不做无目的循环。
...
这一实现借鉴 Anthropic 官方 Agent Quickstart 的“模型—工具结果—继续决策”循环与成对历史裁剪方式，并结合官方 K-12 教案评价量规中的可观察目标、真实课时、文本依赖问题和课堂可扫描性要求；没有照搬 Claude 专属接口，现有系统网关和个人 DeepSeek 仍共享同一套业务编排。
