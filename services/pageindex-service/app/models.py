...
class PageInput(ApiModel):
    ...
    ocr_blocks: list[dict[str, Any]] = Field(default_factory=list, alias="ocrBlocks")
    ocr_error: str | None = Field(default=None, alias="ocrError")

    # Internal ingestion only. The image is used for the OCR attempt and is
    # never persisted in SelectedPage or sent to PageIndex.
    image_base64: str | None = Field(default=None, alias="imageBase64")
    image_mime_type: str = Field(default="image/png", alias="imageMimeType")
...
class PdfIngestRequest(ApiModel):
    """Internal PDF ingestion contract.

    A caller may provide bytes for a private upload or a path under the
    explicitly configured PDF_INPUT_ROOT. Arbitrary URL fetching is not
    supported. The original PDF remains the display source of truth.
    """
