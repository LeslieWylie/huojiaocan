...
class IndexService:
    def __init__(self, repository: FileRepository, adapter: PageIndexAdapter):
    ...
    def start_index(self, command: IndexCommand) -> JobRef:
        ...
        if not pages and existing_index:
            pages = [self._selected_to_input(page) for page in existing_index.pages]
    ...
    def _run_create(self, job: JobRecord, request: CreateIndexRequest) -> None:
        try:
            ...
            job.updated_at = utc_now()
            self.repository.save_job(job)
            pages = [select_page_text(page, request.extraction_policy) for page in request.pages]
            ...
            for page in index_document.pages:
                ...
                scored.append(
                    RetrievedPage(
                        ...
                        providerMetadata={"adapter": self.adapter.name, "qualityScore": page.quality_score},
                    )
                )
    ...
    def validate(self, document_id: str, request: ValidationRequest) -> ValidationReport:
        ...
        for question in request.questions:
            ...
            question_results.append({
                ...
                "hits": [
                    {"pdfPage": hit.pdf_page, "printedPage": hit.printed_page, "sectionPath": hit.section_path, "score": hit.score}
        ...
        elif passed and warnings == 0 and failures == 0:
