...
def create_app(data_root: str | Path | None = None) -> FastAPI:
    ...
    # The public deployment is called by the BFF, not by browsers.  Keep local
    # development and existing tests open when no key is configured, but make a
    # configured deployment fail closed for every endpoint, including health.
